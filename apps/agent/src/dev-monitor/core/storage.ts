...
export async function initStorage(options: InitOptions = {}): Promise<void> {
  ...
  try {
    // Get workspace path
    const { getWorkspacePath } = await import('@/lib/api/backend');
...
async function ensureOutputDir(): Promise<void> {
  ...
  try {
    const { devMonitorEnsureDir } = await import('@/lib/api/backend');
...
async function flushBatch(): Promise<void> {
  ...
  try {
    ...
    const { devMonitorWriteBatch } = await import('@/lib/api/backend');
