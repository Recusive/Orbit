...
<SidebarSection title="Workspaces" collapsible={true} defaultCollapsed={false}>
  {/* Section content */}
</SidebarSection>;
```
...
<InboxButton collapsed={false} onClick={() => console.log('Navigate to inbox')} />;
```
...
<NewConversation collapsed={false} onClick={() => console.log('Create new conversation')} />;
```
...
<WorkspaceItem id="ws-1" name="Personal" defaultExpanded={true}>
  <ConversationItem id="conv-1" title="My Conversation" timestamp={Date.now()} />
</WorkspaceItem>;
```
...
<ConversationItem id="conv-1" title="Chat about React" timestamp={Date.now()} />;
```
...
<PlaygroundSection collapsed={false} onClick={() => console.log('Open playground')} />;
```
