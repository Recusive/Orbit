...
export const MessageSection: FC<MessageSectionProps> = ({
  ...
  return (
    <section data-section-index={sectionIndex} className={`py-4 space-y-4 ${className}`}>
      {messages.map((message) => {
        ...
        // message.type === 'agent'
        return (
          <AgentMessage key={message.id} content={message.content} timestamp={message.timestamp} />
        );
      })}
    </section>
  );
