...
>
> **Plan sync notice**: The companion plan was updated in pass 3 to add `browser_get_url`, `browser_get_title`, `browser_runtime_info` to Phase 2 (~25 new tools, 13 → 38 total), and to correct the screenshot deferral (only annotated overlays are deferred, not basic `browser_screenshot`). **The spec is the source of truth for behavior.**
...
**CSP-degraded snapshot**: When a page's Content Security Policy blocks `orbit_runtime.js` injection, `browser_snapshot` returns a **successful** `SnapshotResponse` with degraded content:

| Field             | Value in CSP-degraded mode                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `emittedElements` | `0`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `truncated`       | `false`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `url`             | Current page URL (obtained via Rust `browser_info` metadata, not JS)                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `title`           | `""` (empty string). **Implementation note**: The current Rust `browser_info` API exposes `url` but not `title`. Adding title requires extending `BrowserInfo` to call `WKWebView.title` (a synchronous WebKit property). Until implemented, title is empty in CSP-degraded mode. This is acceptable because the URL alone is sufficient for the agent to orient, and `browser_eval` (which works even on CSP-blocked pages — see Section 3.9) can read `document.title` as a workaround. |
| `durationMs`      | `0`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |

This is a success response, not an error, so the agent can still read the URL (title is empty until the `BrowserInfo` extension is implemented — see table above). The snapshot text makes the limitation explicit. Subsequent tool calls requiring refs will fail with the standard "Unknown ref" error, guiding the agent to use `selector`-based fallbacks or `browser_eval`.
...
| ------------------------------------- | ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | ---------------------------- |
| `browser_get_url()`                   | None                                       | `{ url: string }` — current `location.href`                                                                       | No-browser only                                                |
| `browser_get_title()`                 | None                                       | `{ title: string }` — current `document.title`                                                                    | No-browser only                                                |
| `browser_get_text(target)`            | `TargetSchema`                             | `{ text: string }` — `element.textContent` (trimmed)                                                              | Standard ref/selector errors                                   |
...
| `browser_eval(script)`         | `script: string`                                | `{ result: unknown }` — return value of the script                      | Evaluates JavaScript in the page context via WKWebView's native `evaluateJavaScript` API. **Not runtime-dependent** — works even on CSP-blocked pages because it bypasses page script restrictions. Subject to transport limits (100,000 chars URL path, 500,000 chars event channel). |
| `browser_console_logs(level?)` | Optional `level: "error"                        | "warn"                                                                  | "info"                                                                                                                                                                                                                                                                                 | "log"` filter | `{ logs: Array<{ level, message, timestamp }> }` | Returns captured console messages. Requires runtime injection for interception. CSP-blocked pages return `"Runtime unavailable"`. |
| `browser_runtime_info()`       | None                                            | `BrowserRuntimeInfo` (see below)                                        | Reports the injected runtime's status. Returns `{ available: false, reason: "No browser open" }` if no browser exists, or `{ available: false, reason: "CSP blocked" }` if runtime couldn't inject.                                                                                    |
...
| CSP blocked          | `"Page CSP blocks script injection. Runtime unavailable."`                                                          | Agent notes limitation to user                                                                   |
