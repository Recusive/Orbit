...
>
> **Plan divergence note**: The companion plan predates this spec and prescribes a different migration boundary (delete 34 Rust commands, move everything to frontend HTTP). This spec is authoritative — it reflects the decision to keep Rust as orchestrator for cold-path operations. The plan must be updated to match this spec's boundary before implementation begins.
...
| Credentials stay in Rust                                        | macOS Keychain access requires native API. `CredentialBridge` unchanged except transport.                                                                                                                         |
...
| Pre-flight config calls per message   | 2-5 Tauri IPC calls                                                                                         | 1 HTTP PUT                                            | DevTools network tab count                                        |
