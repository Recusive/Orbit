...
You are conducting a senior-level code review of Pull Request #{{PR_NUMBER}}. Your goal is to ensure the code is correct, follows best practices, and is production-ready for merge.
</task>

<context>
Review scope: All changes in the pull request comparing {{HEAD_BRANCH}} to {{BASE_BRANCH:main}}.
...
Begin by running \`gh pr view {{PR_NUMBER}}\` to understand the PR context, then \`gh pr diff {{PR_NUMBER}}\` to see all changes. Detect the project context by reading config files. Read related files to understand full context before making any judgments.

Note: Replace {{PR_NUMBER}}, {{HEAD_BRANCH}}, and {{BASE_BRANCH:main}} with actual values.`;
