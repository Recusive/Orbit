/*---------------------------------------------------------------------------------------------
 *  /review-uncommitted - Senior-level code review of all uncommitted changes
 *--------------------------------------------------------------------------------------------*/

export const REVIEW_UNCOMMITTED_PROMPT = `<task>
You are conducting a senior-level code review of all uncommitted changes in this repository. Your goal is to ensure the code is correct, follows best practices, and is production-ready before commit.
</task>

<context>
Review scope: All staged and unstaged changes not yet committed.
...
- Combine both to understand total uncommitted work
...
1. **Review Summary**: What these changes are trying to accomplish
2. **Detected Stack**: Tech stack, frameworks, and standards detected
3. **Files Changed**: List from git status
...
Begin by running \`git status\` to see all changes, then \`git diff\` and \`git diff --cached\` to see the actual modifications. Detect the project context by reading config files. Read related files to understand full context before making any judgments.`;
