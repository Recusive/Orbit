...
Custom tool definitions for the OpenCode project. Each tool is a paired `.ts` (implementation) + `.txt` (description prompt) file. Tools use the `@opencode-ai/plugin` SDK's `tool()` function to define typed arguments, descriptions, and async execute functions. These are project-specific tools loaded at runtime alongside built-in tools.
...
| File                   | Purpose                                                                                                                                                                   |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `github-pr-search.ts`  | Tool implementation — searches GitHub PRs in `anomalyco/opencode` via GitHub Search API. Uses `@opencode-ai/plugin` `tool()` with typed args (`query`, `limit`, `offset`) |
