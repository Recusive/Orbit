...
async function main(): Promise<void> {
  ...
  if (version === undefined) throw new Error("Version argument required, ex: bun script/sync-zed.ts v1.0.52")

  const token = process.env.ZED_EXTENSIONS_PAT
  if (token === undefined) throw new Error("ZED_EXTENSIONS_PAT environment variable required")

  const prToken = process.env.ZED_PR_PAT
  if (prToken === undefined) throw new Error("ZED_PR_PAT environment variable required")
  ...
  const workDir = join(tmpdir(), `zed-extensions-${String(Date.now())}`)
  ...
  if (oldBranches.length > 0) {
    console.log(`🗑️  Found ${String(oldBranches.length)} old branch(es), deleting...`)
  ...
  if (prResult.exitCode !== 0) {
    ...
    throw new Error(`Failed with exit code ${String(prResult.exitCode)}`)
  }
