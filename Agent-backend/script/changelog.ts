...
export async function getLatestRelease(skip?: string): Promise<string> {
  ...
  for (const release of releases) {
    ...
    if (target !== undefined && tag === target) continue
    return tag
  }
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  for (const hash of hashes) {
    ...
    if (/^(ignore:|test:|chore:|ci:|release:)/i.exec(message) !== null) continue
...
function filterRevertedCommits(commits: Commit[]): Commit[] {
  ...
  for (const commit of commits) {
    const match = revertPattern.exec(commit.message)
    if (match !== null) {
      // It's a revert - remove the original if we've seen it
      const original = match[1]
      if (original !== undefined && seen.has(original)) seen.delete(original)
...
export async function generateChangelog(
  ...
  for (let i = 0; i < commits.length; i++) {
    ...
    const summary = summaries[i] ?? commit.message
...
export async function getContributors(from: string, to: string): Promise<Map<string, Set<string>>> {
  const fromRef = from.startsWith("v") ? from : `v${from}`
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
  ...
  for (const line of compare.split("\n").filter(Boolean)) {
    ...
    if (/^(ignore:|test:|chore:|ci:|release:)/i.exec(title) !== null) continue

    if (login !== null && !Script.team.includes(login)) {
      if (!contributors.has(login)) contributors.set(login, new Set())
      const commits = contributors.get(login)
      if (commits !== undefined) commits.add(title)
    }
  }
