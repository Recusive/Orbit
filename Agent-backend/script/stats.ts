...
async function sendToPostHog(event: string, properties: Record<string, unknown>): Promise<void> {
  ...
  if (response !== null && !response.ok) {
    console.warn(`PostHog API error: ${String(response.status)}`)
  }
}
...
async function fetchNpmDownloads(packageName: string): Promise<number> {
  try {
    ...
    const response = await fetch(
      `https://api.npmjs.org/downloads/range/2020-01-01:${String(endYear)}-12-31/${packageName}`,
    )
    if (!response.ok) {
      console.warn(`Failed to fetch npm downloads for ${packageName}: ${String(response.status)}`)
      return 0
    }
    const data: NpmDownloadsRange = (await response.json()) as NpmDownloadsRange
...
async function fetchReleases(): Promise<Release[]> {
  ...
  for (;;) {
    const url = `https://api.github.com/repos/anomalyco/opencode/releases?page=${String(page)}&per_page=${String(per)}`
    ...
    const batch: Release[] = (await response.json()) as Release[]
    if (batch.length === 0) break

    releases.push(...batch)
    console.log(`Fetched page ${String(page)} with ${String(batch.length)} releases`)
...
async function save(githubTotal: number, npmDownloads: number): Promise<void> {
  const file = "STATS.md"
  ...
  try {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      if (line.startsWith("|") && !line.includes("Date") && !line.includes("---")) {
        const match =
          /\|\s*[\d-]+\s*\|\s*([\d,]+)\s*(?:\([^)]*\))?\s*\|\s*([\d,]+)\s*(?:\([^)]*\))?\s*\|\s*([\d,]+)\s*(?:\([^)]*\))?\s*\|/.exec(
            line,
          )
        if (match) {
...
console.log(`\nFetched ${String(releases.length)} releases total\n`)
...
console.log("-".repeat(60))
console.log(`GitHub Total: ${githubTotal.toLocaleString()} downloads across ${String(releases.length)} releases`)
