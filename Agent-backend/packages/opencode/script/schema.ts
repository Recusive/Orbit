...
function generate(schema: z.ZodType): Record<string, unknown> {
  const result = z.toJSONSchema(schema, {
    ...
    override(ctx) {
      ...
      if ("type" in jsonSchema && jsonSchema.type === "string" && typeof jsonSchema.default === "string") {
        jsonSchema.examples ??= [jsonSchema.default]

        jsonSchema.description = [jsonSchema.description ?? "", `default: \`${jsonSchema.default}\``]
          .filter(Boolean)
          .join("\n\n")
          .trim()
      }
