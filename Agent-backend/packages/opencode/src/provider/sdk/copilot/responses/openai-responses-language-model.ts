...
export class OpenAIResponsesLanguageModel implements LanguageModelV2 {
  ...
  private async getArgs({
    ...
    function addInclude(key: OpenAIResponsesIncludeValue): void {
      include = include !== null && include !== undefined ? [...include, key] : [key]
    }

    function hasOpenAITool(id: string): boolean {
      return tools?.find((tool) => tool.type === "provider-defined" && tool.id === id) !== undefined
    }
    ...
    const webSearchToolName: string | undefined = (
      tools?.find(
        (tool) =>
    ...
    const baseArgs = {
      ...
      metadata: openaiOptions?.metadata as Record<string, string> | undefined,
      parallel_tool_calls: openaiOptions?.parallelToolCalls,
      previous_response_id: openaiOptions?.previousResponseId,
      store: openaiOptions?.store,
      user: openaiOptions?.user,
      instructions: openaiOptions?.instructions,
      service_tier: openaiOptions?.serviceTier as string | undefined,
      include,
      ...
      ...(modelConfig.isReasoningModel &&
        (openaiOptions?.reasoningEffort !== undefined || openaiOptions?.reasoningSummary !== undefined) && {
          reasoning: {
            ...(openaiOptions.reasoningEffort !== undefined && {
    ...
    if (modelConfig.isReasoningModel) {
      ...
      if (baseArgs.temperature !== undefined) {
      ...
      if (openaiOptions?.reasoningSummary !== undefined) {
        warnings.push({
          type: "unsupported-setting",
  ...
  async doGenerate(
    ...
    const logprobs: z.infer<typeof LOGPROBS_SCHEMA>[] = []
      ...
      switch (part.type) {
        ...
        case "message": {
          for (const contentPart of part.content) {
            if (
              options.providerOptions?.openai !== undefined &&
              (options.providerOptions.openai as Record<string, unknown>).logprobs !== undefined &&
              contentPart.logprobs !== null &&
              contentPart.logprobs !== undefined
            ) {
        ...
        case "web_search_call": {
          const webSearchId: string = part.id
          const webSearchAction: unknown = part.action
          const webSearchStatus: string = part.status
          content.push({
            type: "tool-call",
            toolCallId: webSearchId,
            toolName: webSearchToolName ?? "web_search",
            input: JSON.stringify({ action: webSearchAction }),
            providerExecuted: true,
          })
  ...
  async doStream(
    ...
    const configGenerateId = this.config.generateId
          ...
          transform(chunk, controller) {
            ...
            if (isResponseOutputItemAddedChunk(value)) {
              if (value.item.type === "function_call") {
                ...
                const webSearchItemId: string = value.item.id
                ongoingToolCalls[value.output_index] = {
                  toolName: webSearchToolName ?? "web_search",
              ...
              if (value.item.type === "function_call") {
                ...
                if (value.output_index in activeReasoning) {
                  const activeReasoningPart = activeReasoning[value.output_index]
                  for (const summaryIndex of activeReasoningPart.summaryParts) {
                    controller.enqueue({
                      type: "reasoning-end",
                      id: `${activeReasoningPart.canonicalId}:${String(summaryIndex)}`,
                      providerMetadata: {
                        openai: {
                  ...
                  Reflect.deleteProperty(activeReasoning, value.output_index)
              ...
              if (toolCall !== undefined) {
                ...
                const codeInterpreterContainerId = toolCall.codeInterpreter?.containerId ?? ""
                controller.enqueue({
                  type: "tool-call",
                  ...
                  input: JSON.stringify({
                    code: value.code,
                    containerId: codeInterpreterContainerId,
                  } satisfies z.infer<typeof codeInterpreterInputSchema>),
                  providerExecuted: true,
                })
              }
              ...
              if (activeItem && value.summary_index > 0) {
                ...
                controller.enqueue({
                  type: "reasoning-start",
                  id: `${activeItem.canonicalId}:${String(value.summary_index)}`,
                  providerMetadata: {
                    openai: {
              ...
              if (value.annotation.type === "url_citation") {
                controller.enqueue({
                  type: "source",
                  sourceType: "url",
                  id: configGenerateId?.() ?? generateId(),
...
interface ResponsesModelConfig {
