...
interface OpenAICompatibleTokenUsage {
...
interface OpenAICompatibleChunkDelta {
  role?: "assistant" | null
  content?: string | null
...
interface OpenAICompatibleChunkChoice {
  delta?: OpenAICompatibleChunkDelta | null
  finish_reason?: string | null
}

interface OpenAICompatibleChunkData {
  id?: string | null
  created?: number | null
  model?: string | null
  choices: OpenAICompatibleChunkChoice[]
  usage?: OpenAICompatibleTokenUsage | null
}
...
export interface OpenAICompatibleChatConfig {
  ...
  errorStructure?: ProviderErrorStructure<z.core.$ZodType>
...
export class OpenAICompatibleChatLanguageModel implements LanguageModelV2 {
  ...
  constructor(modelId: OpenAICompatibleChatModelId, config: OpenAICompatibleChatConfig) {
    ...
    // ProviderErrorStructure uses Zod v4's ZodType which satisfies StandardSchemaV1 (a FlexibleValidator variant),
    // but the generic parameter causes a structural mismatch with @ai-sdk/provider-utils' FlexibleValidator type.
    this.failedResponseHandler = createJsonErrorResponseHandler(
      errorStructure as unknown as Parameters<typeof createJsonErrorResponseHandler>[0],
    )
  ...
  private async getArgs({
    ...
    if (responseFormat?.type === "json" && responseFormat.schema !== undefined && !this.supportsStructuredOutputs) {
      warnings.push({
      ...
      args: {
        ...
        response_format:
          responseFormat?.type === "json"
            ? this.supportsStructuredOutputs && responseFormat.schema !== undefined
              ? {
                  type: "json_schema",
                  json_schema: {
  ...
  async doGenerate(
    ...
    // tool calls:
    if (choice.message.tool_calls !== null && choice.message.tool_calls !== undefined) {
    ...
    if (
      ...
      completionTokenDetails?.accepted_prediction_tokens !== undefined
    ) {
    ...
    if (
      ...
      completionTokenDetails?.rejected_prediction_tokens !== undefined
    ) {
      ...
      stream: response.pipeThrough(
        new TransformStream<ParseResult<z.infer<typeof this.chunkSchema>>, LanguageModelV2StreamPart>({
          ...
          transform(chunk, controller) {
            ...
            const rawValue: unknown = chunk.value
            ...
            if (typeof rawValue === "object" && rawValue !== null && "error" in rawValue) {
              finishReason = "error"
              const errorObj = rawValue as { error: { message: string } }
              controller.enqueue({ type: "error", error: errorObj.error.message })
              return
            }

            const value = rawValue as OpenAICompatibleChunkData
            ...
            if (value.usage !== null && value.usage !== undefined) {
              ...
              if (
                ...
                completion_tokens_details?.accepted_prediction_tokens !== undefined
              ) {
              ...
              if (
                ...
                completion_tokens_details?.rejected_prediction_tokens !== undefined
              ) {
                usage.completionTokensDetails.rejectedPredictionTokens =
                  completion_tokens_details.rejected_prediction_tokens
              }
              if (prompt_tokens_details?.cached_tokens !== null && prompt_tokens_details?.cached_tokens !== undefined) {
            ...
            if (choice.delta === null || choice.delta === undefined) {
              return
            }
            ...
            if (delta.reasoning_opaque !== null && delta.reasoning_opaque !== undefined) {
            ...
            if (delta.content !== null && delta.content !== undefined) {
              ...
              if (isActiveReasoning && !isActiveText) {
                controller.enqueue({
                  type: "reasoning-end",
                  id: "reasoning-0",
                  providerMetadata: reasoningOpaque !== undefined ? { copilot: { reasoningOpaque } } : undefined,
                })
          ...
          flush(controller) {
            ...
            const providerMetadata: SharedV2ProviderMetadata = {
              [providerOptionsName]: {},
              // Include reasoning_opaque for Copilot multi-turn reasoning
              ...(reasoningOpaque !== undefined ? { copilot: { reasoningOpaque } } : {}),
...
): z.ZodUnion<[z.ZodObject<Record<string, z.core.$ZodType>>, ERROR_SCHEMA]> =>
  z.union([
    z.object({
      id: z.string().nullish(),
      created: z.number().nullish(),
