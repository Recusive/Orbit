...
export namespace ProviderTransform {
    ...
    if (typeof model.capabilities.interleaved === "object") {
      ...
      return msgs.map((msg): ModelMessage => {
        if (msg.role === "assistant" && Array.isArray(msg.content)) {
          ...
          if (reasoningText !== "") {
            const existingOptions = (msg.providerOptions ?? {}) as Record<string, Record<string, unknown>>
            return {
              ...msg,
              ...
              providerOptions: {
                ...msg.providerOptions,
                openaiCompatible: {
                  ...existingOptions.openaiCompatible,
                  [field]: reasoningText,
                },
              },
            } as ModelMessage
  ...
  function applyCaching(msgs: ModelMessage[], model: Provider.Model): ModelMessage[] {
    ...
    for (const msg of unique([...system, ...final])) {
      ...
      const shouldUseContentOptions =
        !useMessageLevelOptions && Array.isArray(msg.content) && (msg.content as unknown[]).length > 0

      if (shouldUseContentOptions) {
        const lastContent = msg.content[(msg.content as unknown[]).length - 1]
  ...
  function unsupportedParts(msgs: ModelMessage[], model: Provider.Model): ModelMessage[] {
    return msgs.map((msg) => {
      ...
      const filtered = msg.content.map((part) => {
        ...
        if (modality === undefined) return part
        if (model.capabilities.input[modality]) return part

        const name = filename !== undefined ? `"${filename}"` : modality
        return {
          type: "text" as const,
  ...
  export function message(msgs: ModelMessage[], model: Provider.Model, options: Record<string, unknown>): ModelMessage[] {
    ...
    if (key !== undefined && key !== model.providerID && model.api.npm !== "@ai-sdk/azure") {
      ...
      msgs = msgs.map((msg): ModelMessage => {
        if (!Array.isArray(msg.content)) return { ...msg, providerOptions: remap(msg.providerOptions) } as ModelMessage
        return {
          ...msg,
  ...
  export function providerOptions(model: Provider.Model, options: Record<string, unknown>): Record<string, unknown> {
    if (model.api.npm === "@ai-sdk/gateway") {
      ...
      const slug = rawSlug !== undefined ? (SLUG_OVERRIDES[rawSlug] ?? rawSlug) : undefined
      const gateway: unknown = options.gateway
      ...
      if (has) {
        if (slug !== undefined) {
          ...
          result[slug] = rest
        } else if (gateway !== undefined && typeof gateway === "object" && !Array.isArray(gateway)) {
  ...
  export function schema(model: Provider.Model, schema: JSONSchema.BaseSchema | JSONSchema7): JSONSchema7 {
    ...
    */
    ...
    if (model.providerID === "google" || model.api.id.includes("gemini")) {
      ...
      const hasCombiner = (node: unknown): boolean =>
        isPlainObject(node) && (Array.isArray(node.anyOf) || Array.isArray(node.oneOf) || Array.isArray(node.allOf))
      const hasSchemaIntent = (node: unknown): boolean => {
      ...
      const sanitizeGemini = (obj: unknown): unknown => {
        ...
        if (result.type !== undefined && result.type !== "object" && !hasCombiner(result)) {
