...
export namespace Worktree {
  ...
  function failed(result: { stdout?: Uint8Array; stderr?: Uint8Array }): string[] {
    return [outputText(result.stderr), outputText(result.stdout)].filter(Boolean).flatMap((chunk) =>
      chunk
        .split("\n")
        .map((line) => line.trim())
        .flatMap((line) => {
          const match = /^warning:\s+failed to remove\s+(.+):\s+/i.exec(line)
          if (!match) return []
  ...
  async function prune(root: string, entries: string[]): Promise<void> {
  ...
  async function sweep(root: string): Promise<{ exitCode: number; stdout?: Uint8Array; stderr?: Uint8Array }> {
  ...
  async function candidate(root: string, base?: string): Promise<Info> {
  ...
  async function runStartScript(directory: string, cmd: string, kind: StartKind): Promise<boolean> {
    const text = cmd.trim()
    if (text === "") return true
  ...
  export async function createFromInfo(info: Info, startCommand?: string): Promise<() => void> {
    ...
    try {
      ...
      // Ignore sandbox registration failures
    }
  ...
  export const remove = fn(RemoveInput, async (input) => {
    ...
    const clean = (target: string): Promise<void> =>
      fs
        .rm(target, {
          recursive: true,
          force: true,
    ...
    const stop = async (target: string): Promise<void> => {
  ...
  export const reset = fn(ResetInput, async (input) => {
    ...
    const entry = await (async (): Promise<{ path?: string; branch?: string } | undefined> => {
      for (const item of entries) {
        if (!item.path) continue
        const key = await canonical(item.path)
        if (key === directory) return item
