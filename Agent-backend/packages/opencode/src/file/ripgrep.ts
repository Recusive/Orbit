...
export namespace Ripgrep {
  ...
  const state = lazy(async () => {
    ...
    if (!Filesystem.exists(filepath)) {
      ...
      const config = PLATFORM[platformKey as keyof typeof PLATFORM] as
        | (typeof PLATFORM)[keyof typeof PLATFORM]
        | undefined
      if (config === undefined) throw new UnsupportedPlatformError({ platform: platformKey })
      ...
      if (config.extension === "zip") {
        ...
        const entries = (await zipFileReader.getEntries()) as ZipEntry[]
        let rgEntry: ZipEntry | undefined
  ...
  export async function tree(input: { cwd: string; limit?: number; signal?: AbortSignal }): Promise<string> {
    ...
    if (total > used) lines.push(`[${String(total - used)} truncated]`)
