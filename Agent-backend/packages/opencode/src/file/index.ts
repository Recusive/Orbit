...
export namespace File {
  ...
  const state: () => FileState = Instance.state((): FileState => {
    let cache: FileCache = { files: [], dirs: [] }
    let fetching = false

    const isGlobalHome = Instance.directory === Global.Path.home && Instance.project.id === "global"

    const populate = async (result: FileCache): Promise<void> => {
      ...
      if (isGlobalHome) {
        ...
        const shouldIgnoreNested = (name: string): boolean => name.startsWith(".") || ignoreNested.has(name)
  ...
  export async function read(file: string): Promise<Content> {
    ...
    if (isBinaryByExtension(file) && !isText) {
    ...
    const encode = isText ? false : shouldEncode(mimeType)
  ...
  export async function list(dir?: string): Promise<Node[]> {
    ...
    const project = Instance.project
    let ignored: (p: string) => boolean = () => false
    if (project.vcs === "git") {
      const ig = ignore()
  ...
  export function search(input: {
    ...
    const hidden = (item: string): boolean => {
      const normalized = item.replaceAll("\\", "/").replace(/\/+$/, "")
      return normalized.split("/").some((p) => p.startsWith(".") && p.length > 1)
    }
    const preferHidden = query.startsWith(".") || query.includes("/.")
    const sortHiddenLast = (items: string[]): string[] => {
