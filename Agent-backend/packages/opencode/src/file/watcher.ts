...
export namespace FileWatcher {
  ...
  const watcher = lazy((): typeof ParcelWatcher | undefined => {
    try {
      // eslint-disable-next-line @typescript-eslint/no-require-imports -- @parcel/watcher requires dynamic binding resolution at runtime
    ...
    async () => {
      ...
      if (Instance.project.vcs === "git") {
        ...
        if (vcsDir && !cfgIgnores.includes(".git") && !cfgIgnores.includes(vcsDir)) {
          const gitDirContents = await readdir(vcsDir).catch(() => [] as string[])
