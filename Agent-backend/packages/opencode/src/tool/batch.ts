...
export const BatchTool = Tool.define("batch", {
    ...
    async execute(params, ctx) {
      ...
      const executeCall = async (call: (typeof toolCalls)[0]): Promise<
        | { success: true; tool: string; result: { title: string; output: string; metadata: Record<string, unknown>; attachments?: ToolAttachment[] } }
      ...
      const outputMessage =
        failedCalls > 0
          ? `Executed ${String(successfulCalls)}/${String(results.length)} tools successfully. ${String(failedCalls)} failed.`
          : `All ${String(successfulCalls)} tools executed successfully.\n\nKeep using the batch tool for optimal performance in your next response!`

      const allAttachments: ToolAttachment[] = []
      for (const r of results) {
        if (r.success && r.result.attachments !== undefined) {
          allAttachments.push(...r.result.attachments)
        }
      }

      return {
        title: `Batch execution (${String(successfulCalls)}/${String(results.length)} successful)`,
