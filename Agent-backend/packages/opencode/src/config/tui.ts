...
export namespace TuiConfig {
  ...
  const state = Instance.state(async () => {
    ...
    const projectFiles = Flag.OPENCODE_DISABLE_PROJECT_CONFIG
      ? []
      : ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
  ...
  async function loadFile(filepath: string): Promise<Info> {
    ...
    if (!text) return {}
    return load(text, filepath).catch((error: unknown) => {
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    const data: unknown = await ConfigPaths.parseText(text, configFilepath, "empty")
    if (data === null || data === undefined || typeof data !== "object" || Array.isArray(data)) return {}

    // Flatten a nested "tui" key so users who wrote `{ "tui": { ... } }` inside tui.json
    // (mirroring the old orbit.json shape) still get their settings applied.
    const normalized = (() => {
      const copy = { ...(data as Record<string, unknown>) }
      if (!("tui" in copy)) return copy
      if (copy.tui === null || copy.tui === undefined || typeof copy.tui !== "object" || Array.isArray(copy.tui)) {
        delete copy.tui
        return copy
      }
