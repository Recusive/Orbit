...
export namespace Config {
  ...
  function extractFrontmatterMessage(err: unknown): string | undefined {
    if (
      ...
      const data = (err as Record<string, unknown>).data as Record<string, unknown>
  ...
  function systemManagedConfigDir(): string {
    ...
    if (platform === "darwin") return "/Library/Application Support/orbit"
    if (platform === "win32") return path.join(process.env.ProgramData ?? "C:\\ProgramData", "orbit")
    return "/etc/orbit"
  }
  ...
  export const state = Instance.state(async () => {
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        if (!response.ok) {
          throw new Error(`failed to fetch remote config from ${url}: ${String(response.status)}`)
        }
        ...
        if (typeof remoteConfig.$schema !== "string") remoteConfig.$schema = "https://opencode.ai/config.json"
    ...
    if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
      for (const file of ConfigPaths.projectFiles("orbit", Instance.directory, Instance.worktree)) {
  ...
  async function loadCommand(dir: string): Promise<Record<string, Command>> {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.orbit/command/", "/.orbit/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string): Promise<Record<string, Agent>> {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.orbit/agent/", "/.orbit/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  async function loadMode(dir: string): Promise<Record<string, Agent>> {
    const result: Record<string, Agent> = {}
  ...
  async function loadPlugin(dir: string): Promise<string[]> {
  ...
  const permissionPreprocess = (val: unknown): unknown => {
  ...
  export const Keybinds = z
    .object({
      ...
      filetree_toggle: z.string().optional().default("<leader>f").describe("Toggle file tree"),
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: orbit.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for orbit serve and web commands"),
      command: z
      ...
      lsp: z
        ...
        .refine(
          (data) => {
            if (data === undefined || data === false) return true
      ...
      await import(pathToFileURL(legacy).href, {
        ...
        .then(async (mod: Record<string, unknown>) => {
          ...
          if (defaultExport !== undefined) {
            const { provider, model, ...rest } = defaultExport
  ...
  async function load(text: string, options: { path: string } | { dir: string; source: string }): Promise<Info> {
    const original = text
    const source = "path" in options ? options.path : options.source
    const isFile = "path" in options
    const data: unknown = await ConfigPaths.parseText(
      text,
      "path" in options ? options.path : { source: options.source, dir: options.dir },
    )

    const normalized = ((): unknown => {
      if (data === null || data === undefined || typeof data !== "object" || Array.isArray(data)) return data
      ...
      delete copy.tui
      log.warn("tui keys in orbit config are deprecated; move them to tui.json", { path: source })
      return copy
    })()
        ...
        for (let i = 0; i < result.plugin.length; i++) {
          ...
          try {
            ...
            try {
              ...
              const req = createRequire(options.path)
  ...
  export async function updateGlobal(config: Info): Promise<Info> {
    ...
    const before = await Filesystem.readText(filepath).catch((err: unknown) => {
