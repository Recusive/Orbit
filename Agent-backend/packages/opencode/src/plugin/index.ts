...
export namespace Plugin {
  ...
  const state = Instance.state(async () => {
    ...
    for (const plugin of INTERNAL_PLUGINS) {
      ...
      const init = await plugin(pluginInput).catch((err: unknown) => {
    ...
    for (let plugin of plugins) {
      ...
      if (!plugin.startsWith("file://")) {
        ...
        plugin = await BunProc.install(pkg, version).catch((err: unknown) => {
          ...
          void Bus.publish(Session.Event.Error, {
            ...
            }).toObject() as { name: "UnknownError"; data: { message: string } },
          })
          return ""
        })
        if (!plugin) continue
      ...
      await import(plugin)
        .then(async (mod: Record<string, PluginInstance>) => {
          const seen = new Set<PluginInstance>()
          for (const [, fn] of Object.entries(mod)) {
            if (seen.has(fn)) continue
            seen.add(fn)
            hooks.push(await fn(pluginInput))
          }
        })
        .catch((err: unknown) => {
  ...
  export async function init(): Promise<void> {
    ...
    Bus.subscribeAll((event: unknown) => {
      void state().then((x) => {
        for (const hook of x.hooks) {
          void hook.event?.({
            event: event as SdkEvent,
          })
        }
      })
    })
