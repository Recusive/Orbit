...
function startOAuthServer(): { port: number; redirectUri: string } {
  if (oauthServer) {
    return { port: OAUTH_PORT, redirectUri: `http://localhost:${String(OAUTH_PORT)}/auth/callback` }
  }
  ...
  return Promise.resolve({
    auth: {
      ...
      async loader(getAuth, provider) {
        ...
        const keysToRemove = Object.keys(provider.models).filter(
          (modelId) => !modelId.includes("codex") && !allowedModels.has(modelId),
        )
        for (const key of keysToRemove) {
          Reflect.deleteProperty(provider.models, key)
        }

        if (!("gpt-5.3-codex" in provider.models)) {
          const model = {
            id: "gpt-5.3-codex",
            providerID: "openai",
            api: {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any -- variants() returns Record<string, Record<string, any>> per Provider.Model schema
              ...
              async callback() {
                for (;;) {
                  ...
                  if (response.ok) {
                    ...
                    if (!tokenResponse.ok) {
                      throw new Error(`Token exchange failed: ${String(tokenResponse.status)}`)
                    }
    ...
    "chat.headers": (_input, output) => {
      ...
      output.headers.originator = "orbit"
      output.headers["User-Agent"] = `orbit/${Installation.VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`
