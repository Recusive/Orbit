...
export namespace SessionCompaction {
  ...
  export async function prune(input: { sessionID: string }): Promise<void> {
    ...
    log.info("pruning")
    const msgs = Session.messages({ sessionID: input.sessionID })
    let total = 0
    let pruned = 0
    const toPrune = []
    let turns = 0
    ...
    loop: for (let msgIndex = msgs.length - 1; msgIndex >= 0; msgIndex--) {
      ...
      if (turns < 2) continue
      if (msg.info.role === "assistant" && msg.info.summary === true) break loop
  ...
  export async function process(input: {
    ...
    if (!parentMsg) throw new Error(`Parent message not found: ${input.parentID}`)
    const userMessage = parentMsg.info as MessageV2.User
...
---`
    ...
    if (result === "compact") {
      ...
      }).toObject() as z.infer<typeof MessageV2.ContextOverflowError.Schema>
    ...
    void Bus.publish(Event.Compacted, { sessionID: input.sessionID })
    return "continue"
  }
