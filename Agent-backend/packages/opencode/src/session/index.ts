...
export namespace Session {
  ...
  function createDefaultTitle(isChild = false): string {
  ...
  export function toRow(info: Info): typeof SessionTable.$inferInsert {
    return {
      id: info.id,
  ...
  export async function createNext(input: {
    ...
    Database.use((db) => {
      ...
      Database.effect(() => {
        void Bus.publish(Event.Created, {
          info: result,
        })
      })
    })
  ...
  export function plan(input: { slug: string; time: { created: number } }): string {
  ...
  export const messages = fn(
    ...
    (input) => {
      ...
      for (const msg of MessageV2.stream(input.sessionID)) {
        if (input.limit !== undefined && input.limit !== 0 && result.length >= input.limit) break
        result.push(msg)
      }
      result.reverse()
      return result
  ...
  export const remove = fn(SessionID.zod, async (sessionID) => {
    try {
      ...
      Database.use((db) => {
        ...
        Database.effect(() => {
          void Bus.publish(Event.Deleted, {
            info: session,
          })
        })
      })
  ...
  export const updateMessage = fn(MessageV2.Info, (msg) => {
    ...
    Database.use((db) => {
      db.insert(MessageTable)
        ...
        .run()
      Database.effect(() => {
        void Bus.publish(MessageV2.Event.Updated, {
          info: msg,
        })
      })
    })
    ...
    (input) => {
      ...
      Database.use((db) => {
        db.delete(MessageTable)
          ...
          .run()
        Database.effect(() => {
          void Bus.publish(MessageV2.Event.Removed, {
    ...
    (input) => {
      Database.use((db) => {
        db.delete(PartTable)
          ...
          .run()
        Database.effect(() => {
          void Bus.publish(MessageV2.Event.PartRemoved, {
            sessionID: input.sessionID,
            messageID: input.messageID,
            partID: input.partID,
          })
  ...
  export const updatePart = fn(UpdatePartInput, (part) => {
  ...
  export const updatePartDelta = fn(
    ...
    (input) => {
      void Bus.publish(MessageV2.Event.PartDelta, input)
    },
  )
  ...
  export const getUsage = fn(
    ...
    (input): { cost: number; tokens: MessageV2.Assistant["tokens"] } => {
      ...
      const anthropic = input.metadata?.anthropic as Record<string, unknown> | undefined
      const bedrock = input.metadata?.bedrock as Record<string, unknown> | undefined
      const venice = input.metadata?.venice as Record<string, unknown> | undefined
      ...
      const costInfo =
        input.model.cost.experimentalOver200K !== undefined && tokens.input + tokens.cache.read > 200_000
