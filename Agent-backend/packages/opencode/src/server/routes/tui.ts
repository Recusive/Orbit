...
export async function callTui(ctx: Context): Promise<unknown> {
  ...
  new Hono()
    ...
    .post(
      ...
      async (c) => {
        ...
        const mapped = command in commandMap ? commandMap[command] : command
        await Bus.publish(TuiEvent.CommandExecute, {
          command: mapped,
        })
        return c.json(true)
    ...
    .post(
      ...
      async (c) => {
        ...
        if (eventDef === undefined) throw new Error(`Unknown TUI event type: ${evt.type}`)
