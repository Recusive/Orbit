...
export class McpOAuthProvider implements OAuthClientProvider {
  ...
  get redirectUrl(): string {
    return `http://127.0.0.1:${String(OAUTH_CALLBACK_PORT)}${OAUTH_CALLBACK_PATH}`
  }
  ...
  async invalidateCredentials(type: "all" | "client" | "tokens"): Promise<void> {
    ...
    switch (type) {
      case "all":
        ...
        break
      case "client": {
        const withoutClient = { ...entry }
        delete withoutClient.clientInfo
        await McpAuth.set(this.mcpName, withoutClient)
        break
      }
      case "tokens": {
        const withoutTokens = { ...entry }
        delete withoutTokens.tokens
        await McpAuth.set(this.mcpName, withoutTokens)
        break
      }
    }
  }
