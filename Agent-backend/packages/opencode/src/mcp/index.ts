...
export namespace MCP {
  ...
  function registerNotificationHandlers(client: MCPClient, serverName: string): void {
  ...
  // eslint-disable-next-line @typescript-eslint/no-deprecated -- SSE fallback required for servers that don't support StreamableHTTP yet
  ...
  const state = Instance.state(
    ...
    async (state) => {
      ...
      for (const client of Object.values(state.clients)) {
        const transport = client.transport as Record<string, unknown> | undefined
  ...
  async function fetchPromptsForClient(
    clientName: string,
    client: Client,
  ): Promise<Record<string, PromptInfo & { client: string }> | undefined> {
    const prompts = await client.listPrompts().catch((e: unknown) => {
  ...
  async function fetchResourcesForClient(
    clientName: string,
    client: Client,
  ): Promise<Record<string, ResourceInfo & { client: string }> | undefined> {
    const resources = await client.listResources().catch((e: unknown) => {
  ...
  export async function add(name: string, mcp: Config.Mcp): Promise<{
    ...
    const existingClient = s.clients[name] as MCPClient | undefined
    if (existingClient !== undefined) {
      await existingClient.close().catch((error: unknown) => {
  ...
  async function create(
    ...
    if (mcp.type === "remote") {
      ...
      if (!oauthDisabled) {
        authProvider = new McpOAuthProvider(
          ...
          {
            onRedirect: (_url) => {
              log.info("oauth redirect requested", { key, url: _url.toString() })
      ...
      const transports: { name: string; transport: TransportWithAuth }[] = [
        {
          name: "StreamableHTTP",
      ...
      for (const { name, transport } of transports) {
        try {
          ...
          const isAuthError =
            error instanceof UnauthorizedError || (authProvider !== undefined && lastError.message.includes("OAuth"))
          if (isAuthError) {
            ...
            if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
              ...
              Bus.publish(TuiEvent.ToastShow, {
                ...
                log.debug("failed to show toast", { error: e })
              })
            } else {
    ...
    if (mcp.type === "local") {
      ...
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          ...(["opencode", "orbit"].includes(cmd) ? { BUN_BE_BUN: "1" } : {}),
          ...mcp.environment,
        },
      })
    ...
    const result = await withTimeout(mcpClient.listTools(), mcp.timeout ?? DEFAULT_TIMEOUT).catch((err: unknown) => {
  ...
  export async function connect(name: string): Promise<void> {
    const cfg = await Config.get()
    const config = cfg.mcp ?? {}
    const mcp = config[name] as McpEntry | undefined
    if (mcp === undefined) {
  ...
  export async function disconnect(name: string): Promise<void> {
    const s = await state()
    const client = s.clients[name] as MCPClient | undefined
    if (client !== undefined) {
  ...
  export async function tools(): Promise<Record<string, Tool>> {
    ...
    const connectedClients = Object.entries(clientsSnapshot).filter(
      ([clientName]) => (s.status[clientName] as Status | undefined)?.status === "connected",
    )

    const toolsResults = await Promise.all(
      connectedClients.map(async ([clientName, client]) => {
        const toolsResult = await client.listTools().catch((e: unknown) => {
    ...
    for (const { clientName, client, toolsResult } of toolsResults) {
      ...
      const mcpConfig = config[clientName] as McpEntry | undefined
      const entry = mcpConfig !== undefined && isMcpConfigured(mcpConfig) ? mcpConfig : undefined
  ...
  export async function prompts(): Promise<Record<string, PromptInfo & { client: string }>> {
    const s = await state()
    const clientsSnapshot = await clients()
      ...
      (
        await Promise.all(
          Object.entries(clientsSnapshot).map(async ([clientName, client]) => {
            if ((s.status[clientName] as Status | undefined)?.status !== "connected") {
              return []
            }
  ...
  export async function resources(): Promise<Record<string, ResourceInfo & { client: string }>> {
    const s = await state()
    const clientsSnapshot = await clients()
  ...
  ): Promise<Awaited<ReturnType<MCPClient["getPrompt"]>> | undefined> {
    const clientsSnapshot = await clients()
    const client = clientsSnapshot[clientName] as MCPClient | undefined
  ...
  export async function readResource(
    clientName: string,
    resourceUri: string,
  ): Promise<Awaited<ReturnType<MCPClient["readResource"]>> | undefined> {
    const clientsSnapshot = await clients()
    const client = clientsSnapshot[clientName] as MCPClient | undefined
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        subprocess.on("exit", (code) => {
          if (code !== null && code !== 0) {
            clearTimeout(timeout)
            reject(new Error(`Browser open failed with exit code ${String(code)}`))
          }
        })
      })
    } catch (error) {
      ...
      void Bus.publish(BrowserOpenFailed, { mcpName, url: authorizationUrl })
    }
