...
export namespace ACP {
  ...
  async function sendUsageUpdate(
    ...
    const used = msg.tokens.input + msg.tokens.cache.read
  ...
  export function init(_: { sdk: OpencodeClient }): {
    create: (connection: AgentSideConnection, fullConfig: ACPConfig) => Agent
  } {
    void _
    return {
      create: (connection: AgentSideConnection, fullConfig: ACPConfig): Agent => {
  ...
  export class Agent implements ACPAgent {
    ...
    private async runEventSubscription(): Promise<void> {
      for (;;) {
        ...
        for await (const event of events.stream) {
          await this.handleEvent(event.payload).catch((error: unknown) => {
        ...
        const next = prev
          .then(async () => {
            ...
            if (res.outcome.optionId !== "reject" && permission.permission === "edit") {
              ...
              const content = Filesystem.exists(filepath) ? await Filesystem.readText(filepath) : ""
    ...
    authenticate(_: AuthenticateRequest): never {
      void _
    ...
    async loadSession(params: LoadSessionRequest): Promise<{
      ...
      try {
        ...
        if (lastUser?.role === "user") {
          ...
          if (result.modes?.availableModes.some((m) => m.id === lastUser.agent) === true) {
    ...
    async unstable_listSessions(params: ListSessionsRequest): Promise<ListSessionsResponse> {
      try {
        ...
        const next = filtered.length > limit && page.length > 0 ? String(page[page.length - 1].time.updated) : undefined

        const response: ListSessionsResponse = {
          sessions: entries,
        }
        if (next !== undefined) response.nextCursor = next
        return response
      } catch (e) {
        const error = MessageV2.fromError(e, {
          providerID: this.config.defaultModel?.providerID ?? "unknown",
    ...
    private async processMessage(message: SessionMessageResponse): Promise<void> {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
              ...
              .sessionUpdate({
                ...
                update: {
                  ...
                  content: {
                    type: "text",
                    text: part.text,
                    ...(audience !== undefined && { annotations: { audience } }),
                  },
                },
              })
              .catch((err: unknown) => {
          ...
          const mime = part.mime !== "" ? part.mime : "application/octet-stream"
          ...
          if (url.startsWith("file://")) {
            ...
            const base64Match = /^data:([^;]+);base64,(.*)$/.exec(url)
    ...
    private bashOutput(part: ToolPart): string | undefined {
      if (part.tool !== "bash") return undefined
      if (!("metadata" in part.state) || part.state.metadata === undefined || typeof part.state.metadata !== "object")
        return undefined
    ...
    private async toolStart(sessionId: string, part: ToolPart): Promise<void> {
    ...
    private async loadSessionMode(params: LoadSessionRequest): Promise<{
      sessionId: string
      models: { currentModelId: string; availableModels: ModelOption[] }
      modes: { availableModes: ModeOption[]; currentModeId: string } | undefined
      _meta: ReturnType<typeof buildVariantMeta>
      ...
      const providersData = await this.sdk.config.providers({ directory }).then((x) => x.data)
      const providers = providersData?.providers ?? []
      const entries = sortProvidersByName(providers)
      const availableVariants = modelVariantsFromProviders(entries, model)
      const currentVariant = this.sessionManager.getVariant(sessionId)
      if (currentVariant !== undefined && !availableVariants.includes(currentVariant)) {
    ...
    async unstable_setSessionModel(params: SetSessionModelRequest): Promise<{
      _meta: ReturnType<typeof buildVariantMeta>
    }> {
    ...
    async prompt(params: PromptRequest): Promise<{
      ...
      for (const part of params.prompt) {
        switch (part.type) {
          ...
          case "image": {
            const parsedImage = parseUri(part.uri ?? "")
            const filename = parsedImage.type === "file" ? parsedImage.filename : "image"
            if (part.data !== "") {
              parts.push({
                type: "file",
            ...
            } else if (part.uri?.startsWith("http:") === true) {
              parts.push({
                type: "file",
                url: part.uri,
                filename,
          ...
          case "resource_link": {
            const parsedLink = parseUri(part.uri)
            // Use the name from resource_link if available
            if (part.name !== "" && parsedLink.type === "file") {
              parsedLink.filename = part.name
            }
          ...
          case "audio":
            // Audio prompt parts are not supported
            break
      ...
      const cmd = ((): { name: string; args: string } | undefined => {
        const text = parts
      ...
      const buildUsage = (msg: AssistantMessage): Usage => ({
        ...
        cachedReadTokens: msg.tokens.cache.read !== 0 ? msg.tokens.cache.read : undefined,
        cachedWriteTokens: msg.tokens.cache.write !== 0 ? msg.tokens.cache.write : undefined,
      })
      ...
      if (cmd === undefined) {
        ...
        return {
          ...
          usage: msg !== undefined ? buildUsage(msg) : undefined,
          _meta: {},
        }
      }
  ...
  async function defaultModel(
    ...
    const specified = await sdk.config
      ...
      .then((resp) => {
        const cfg = resp.data
        if (cfg.model === undefined || cfg.model === "") return undefined
    ...
    if (specified !== undefined && providers.length > 0) {
      const provider = providers.find((p) => p.id === specified.providerID)
      if (provider !== undefined && specified.modelID in provider.models) return specified
    }
    ...
    const allModels = providers.flatMap((p) => Object.values(p.models))
    const sortedAll = Provider.sort(allModels as Provider.Model[])
    if (sortedAll.length > 0) {
      return {
        providerID: sortedAll[0].providerID,
        modelID: sortedAll[0].id,
      }
    }
  ...
  function parseUri(
    ...
    try {
      ...
      if (uri.startsWith("zed://")) {
        ...
        if (filePath !== null) {
          const nameSegments = filePath.split("/")
  ...
  function modelVariantsFromProviders(
    ...
    if (modelInfo.variants === undefined) return []
  ...
  function buildAvailableModels(
    ...
    return providers.flatMap((provider) => {
      ...
      return models.flatMap((model) => {
        ...
        if (!includeVariants || model.variants === undefined) return [base]
  ...
  function formatModelIdWithVariant(
    ...
    if (!includeVariant || variant === undefined || !availableVariants.includes(variant)) return base
    return `${base}/${variant}`
  }
  ...
  function parseModelSelection(
    ...
    if (segments.length > 1) {
      ...
      if (baseModelId in provider.models) {
        const baseModelInfo = provider.models[baseModelId]
        if (baseModelInfo.variants !== undefined && candidateVariant in baseModelInfo.variants) {
          return {
