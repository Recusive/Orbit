...
function walk(ast: SchemaAST.AST): z.ZodType {
  const out = body(ast)
...
function body(ast: SchemaAST.AST): z.ZodType {
...
function opt(ast: SchemaAST.AST): z.ZodType {
  ...
  if (items.length > 1) return z.union(items.map(walk) as [z.ZodType, z.ZodType, ...z.ZodType[]]).optional()
  return z.undefined().optional()
}

function union(ast: SchemaAST.Union): z.ZodType {
...
function object(ast: SchemaAST.Objects): z.ZodType {
