...
const TypeId = Symbol.for("@orbit/InstanceState")
...
export namespace InstanceState {
  ...
  export const get = <A, E, R>(self: State<A, E, R>): Effect.Effect<A, E, R> =>
    ScopedCache.get(self.cache, Instance.directory)

  export const has = <A, E, R>(self: State<A, E, R>): Effect.Effect<boolean> =>
    ScopedCache.has(self.cache, Instance.directory)

  export const invalidate = <A, E, R>(self: State<A, E, R>): Effect.Effect<void> =>
    ScopedCache.invalidate(self.cache, Instance.directory)

  export const dispose = (key: string): Effect.Effect<void> =>
    Effect.all(
