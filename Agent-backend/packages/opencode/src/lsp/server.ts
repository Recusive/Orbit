...
export namespace LSPServer {
  ...
  const pathExists = async (p: string): Promise<boolean> =>
    fs
      ...
      .then(() => true)
      .catch(() => false)
  const run = (cmd: string[], opts: Process.RunOptions = {}): Promise<Process.Result> =>
    Process.run(cmd, { ...opts, nothrow: true })
  const output = (cmd: string[], opts: Process.RunOptions = {}): Promise<Process.TextResult> =>
  ...
  const NearestRoot = (includePatterns: string[], excludePatterns?: string[]): RootFunction => {
    return (file) => {
      if (excludePatterns) {
        ...
        const excluded = excludedFiles.next()
        excludedFiles.return(undefined)
        if (excluded.done === false) return Promise.resolve(undefined)
      }
      ...
      files.return(undefined)
      if (first.done !== false) return Promise.resolve(Instance.directory)
      return Promise.resolve(path.dirname(first.value))
    }
  }
  ...
  export const Deno: Info = {
    ...
    root: (file) => {
      ...
      const first = files.next()
      files.return(undefined)
      if (first.done !== false) return Promise.resolve(undefined)
      return Promise.resolve(path.dirname(first.value))
    },
  ...
  export const Typescript: Info = {
    ...
    spawn(root) {
      ...
      if (!tsserver) return Promise.resolve(undefined)
  ...
  export const Oxlint: Info = {
    ...
    async spawn(root) {
      ...
      const resolveBin = (target: string): string | undefined => {
        ...
        candidates.return(undefined)
        if (first.done === false) return first.value

        return undefined
      }

      let lintBin = resolveBin(lintTarget)
      if (!lintBin) {
      ...
      let serverBin = resolveBin(serverTarget)
      if (!serverBin) {
      ...
      if (!bin) {
        ...
        if (platform !== "win32") {
          await fs.chmod(bin, 0o755).catch(() => {
            // noop — best-effort permission change
          })
        }
  ...
  export const Clangd: Info = {
    ...
    async spawn(root) {
      ...
      const assets = release.assets ?? []
      const valid = (item: { name?: string; browser_download_url?: string }): boolean => {
      ...
      await fs.symlink(bin, path.join(Global.Path.bin, "clangd")).catch(() => {
        // noop — symlink may already exist
      })
  ...
  export const JDTLS: Info = {
    ...
    async spawn(root) {
      ...
      const javaMajorVersion = await run(["java", "-version"]).then((result) => {
        ...
        return m === null ? undefined : parseInt(m[1])
      })
      if (javaMajorVersion === undefined || javaMajorVersion < 21) {
      ...
      const jarFileName =
        (await fs.readdir(launcherDir).catch(() => [] as string[]))
      ...
      const configFile = path.join(
        ...
        (() => {
          ...
          if (plat === "darwin") return "config_mac"
          if (plat === "linux") return "config_linux"
          if (plat === "win32") return "config_win"
          return "config_linux"
        })(),
      )
  ...
  export const LuaLS: Info = {
    ...
    async spawn(root) {
      ...
      if (!bin) {
        ...
        assetName = `lua-language-server-${tagName}-${lualsPlatform}-${lualsArch}.${ext}`
  ...
  export const TerraformLS: Info = {
    ...
    async spawn(root) {
      ...
      if (!bin) {
        ...
        if (!build?.url) {
          log.error(
            `Could not find build for ${tfPlatform}/${tfArch} terraform-ls release version ${release.version ?? "unknown"}`,
          )
          return
        }
