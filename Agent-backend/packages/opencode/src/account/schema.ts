...
export const AccountID = Schema.String.pipe(
  ...
  withStatics((schema) => ({ make: (id: string) => schema.makeUnsafe(id) })),
)
...
export const AccessToken = Schema.String.pipe(
  ...
  withStatics((schema) => ({ make: (token: string) => schema.makeUnsafe(token) })),
)
...
export const DeviceCode = Schema.String.pipe(
  ...
  withStatics((schema) => ({ make: (code: string) => schema.makeUnsafe(code) })),
)
