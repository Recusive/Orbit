...
import { Account, AccountRepoError } from "./schema"
...
export class AccountRepo extends ServiceMap.Service<AccountRepo, AccountRepo.Service>()("@opencode/AccountRepo") {
  static readonly layer: Layer.Layer<AccountRepo> = Layer.effect(
    ...
    Effect.sync(() => {
      ...
      const query = <A>(f: (db: DbClient) => A): Effect.Effect<A, AccountRepoError> =>
        Effect.try({
          try: () => Database.use(f),
          catch: (cause) => new AccountRepoError({ message: "Database operation failed", cause }),
        })

      const tx = <A>(f: (db: DbClient) => A): Effect.Effect<A, AccountRepoError> =>
        Effect.try({
          try: () => Database.transaction(f),
          catch: (cause) => new AccountRepoError({ message: "Database operation failed", cause }),
        })

      const current = (db: DbClient): (AccountRow & { active_org_id: OrgID | null }) | undefined => {
      ...
      const state = (db: DbClient, accountID: AccountID, orgID: Option.Option<OrgID>): void => {
      ...
      const persistToken = Effect.fn("AccountRepo.persistToken")((input: AccountRepo.PersistTokenInput) =>
        query((db) => {
      ...
      const persistAccount = Effect.fn("AccountRepo.persistAccount")((input: AccountRepo.PersistAccountInput) =>
        tx((db) => {
