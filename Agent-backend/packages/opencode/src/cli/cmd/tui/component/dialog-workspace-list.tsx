...
async function openWorkspace(input: {
  ...
  const cacheSession = (session: Session): void => {
    input.sync.set(
      "session",
...
function DialogWorkspaceCreate(props: { onSelect: (workspaceID: string) => Promise<void> }): JSX.Element {
  ...
  const createWorkspace = async (type: string): Promise<void> => {
    if (creating()) return
    setCreating(type)

    const result = await sdk.client.experimental.workspace.create({ type, branch: null }).catch((err: unknown) => {
      console.log(err)
      return undefined
    })
...
export function DialogWorkspaceList(): JSX.Element {
  const dialog = useDialog()
  ...
  const open = (workspaceID: string, forceCreate?: boolean): Promise<void> =>
    openWorkspace({
      dialog,
      route,
      sdk,
  ...
  const options = createMemo(() => [
    {
      ...
      footer: `${String(localCount())} session${localCount() === 1 ? "" : "s"}`,
    },
        ...
        footer:
          count === undefined
            ...
            : count === null
              ...
              : `${String(count)} session${count === 1 ? "" : "s"}`,
      }
    }),
    {
      title: "+ New workspace",
