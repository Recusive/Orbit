...
export function Prompt(props: PromptProps): JSX.Element {
  ...
  function promptModelWarning(): void {
    toast.show({
      variant: "warning",
  ...
  const fileStyleId = syntax().getStyleId("extmark.file") ?? 0
  const agentStyleId = syntax().getStyleId("extmark.agent") ?? 0
  const pasteStyleId = syntax().getStyleId("extmark.paste") ?? 0
  ...
  command.register(() => {
    return [
      ...
      },
      {
        ...
        onSelect: (dialog) => {
          if (autocomplete.visible !== false) return
          if (!input.focused) return
  ...
  function restoreExtmarksFromParts(parts: PromptInfo["parts"]): void {
    input.extmarks.clear()
    setStore("extmarkToPartIndex", new Map())
  ...
  function syncExtmarksWithPromptParts(): void {
  ...
  function pasteText(text: string, virtualText: string): void {
  ...
  function pasteImage(file: { filename?: string; content: string; mime: string }): void {
    const currentOffset = input.visualCursor.offset
    const extmarkStart = currentOffset
    const count = store.prompt.parts.filter((x) => x.type === "file" && x.mime.startsWith("image/")).length
    const virtualText = `[Image ${String(count + 1)}]`
    const extmarkEnd = extmarkStart + virtualText.length
            ...
            <textarea
              ...
              onPaste={(event: PasteEvent) => {
                ...
                if (!isUrl) {
                  try {
                    ...
                    if (mime.startsWith("image/")) {
                      event.preventDefault()
                      void Filesystem.readArrayBuffer(filepath)
                ...
                if (
                  ...
                  event.preventDefault()
                  pasteText(pastedContent, `[Pasted ~${String(lineCount)} lines]`)
                  return
                }
                  ...
                  {(() => {
                    ...
                    onMount(() => {
                      const timer = setInterval(() => {
                        ...
                        if (next !== undefined && next !== 0) setSeconds(Math.round((next - Date.now()) / 1000))
                      }, 1000)
                    ...
                    const retryText = (): string => {
                      ...
                      const retryInfo = ` [retrying ${duration ? `in ${duration} ` : ""}attempt #${String(r.attempt)}]`
