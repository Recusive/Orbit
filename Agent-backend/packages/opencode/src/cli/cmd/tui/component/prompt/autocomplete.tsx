...
function removeLineRange(input: string): string {
  const hashIndex = input.lastIndexOf("#")
  return hashIndex !== -1 ? input.substring(0, hashIndex) : input
}

function extractLineRange(input: string): {
  lineRange?: { baseName: string; startLine: number; endLine: number | undefined }
  baseQuery: string
} {
  const hashIndex = input.lastIndexOf("#")
  if (hashIndex === -1) {
  ...
  const lineMatch = /^(\d+)(?:-(\d*))?$/.exec(linePart)
...
export function Autocomplete(props: {
  ...
  const position = createMemo(() => {
    if (store.visible === false) return { x: 0, y: 0, width: 0 }
  ...
  function insertPart(text: string, part: PromptInfo["parts"][number]): void {
  ...
  const [files] = createResource(
    () => search(),
    async (query) => {
      if (store.visible === false || store.visible === "/") return []
        ...
        options.push(
          ...sortedFiles.map((item): AutocompleteOption => {
            ...
            if (lineRange && !item.endsWith("/")) {
              filename = `${item}#${String(lineRange.startLine)}${lineRange.endLine !== undefined ? `-${String(lineRange.endLine)}` : ""}`
  ...
  const commands = createMemo((): AutocompleteOption[] => {
    ...
    const maxLen = firstBy(results, [(x) => x.display.length, "desc"])?.display.length
    if (typeof maxLen !== "number" || maxLen === 0) return results
  ...
  const options = createMemo((prev: AutocompleteOption[] | undefined) => {
    ...
    const result = fuzzysort.go(removeLineRange(searchValue), mixed, {
      ...
      scoreFn: (objResults) => {
        ...
        if (displayResult.target.startsWith(String(store.visible) + searchValue)) {
          score *= 2
        }
  ...
  function select(): void {
    const selected = options()[store.selected] as AutocompleteOption | undefined
    if (!selected) return
    hide()
    selected.onSelect?.()
  }

  function expandDirectory(): void {
    const selected = options()[store.selected] as AutocompleteOption | undefined
    if (!selected) return
    ...
    props.ref({
      ...
      onKeyDown(e: KeyEvent) {
        if (store.visible !== false) {
          ...
          if (name === "tab") {
            const sel = options()[store.selected] as AutocompleteOption | undefined
            if (sel?.isDirectory) {
              expandDirectory()
            } else {
              select()
  ...
  const height = createMemo(() => {
    ...
    if (store.visible === false) return Math.min(10, count)
    positionTick()
