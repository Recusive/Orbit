...
export function DialogStash(props: { onSelect: (entry: StashEntry) => void }): JSX.Element {
    ...
    return entries
      .map((entry, index) => {
        ...
        return {
          ...
          footer: lineCount > 1 ? `~${String(lineCount)} lines` : undefined,
        }
      })
      .toReversed()
  })
  ...
  return (
    <DialogSelect
      ...
      onSelect={(option) => {
        const entries = stash.list()
        const entry = entries[option.value] as StashEntry | undefined
        if (entry !== undefined) {
