...
import orbit from "./theme/orbit.json" with { type: "json" }
...
function resolveTheme(theme: ThemeJson, mode: "dark" | "light"): Theme {
  ...
  function resolveColor(c: ColorValue): RGBA {
    ...
    if (typeof c === "string") {
      ...
      if (c in defs) {
        ...
        return resolveColor(theme.theme[c as keyof ThemeColors] as ColorValue)
      } else {
  ...
  if (selectedListItemTextValue !== undefined) {
  ...
  return {
    ...resolved,
    _hasSelectedListItemText: selectedListItemTextValue !== undefined,
    thinkingOpacity,
  } as Theme
}
...
function ansiToRgba(code: number): RGBA {
  ...
  if (code < 232) {
    ...
    const val = (x: number): number => (x === 0 ? 0 : x * 40 + 55)
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const [store, setStore] = createStore({
      ...
      active: config.theme ?? kv.get<string>("theme", "orbit"),
      ready: false,
    })
    ...
    function resolveSystemTheme(): void {
    ...
    return {
      theme: new Proxy(values(), {
        get(_target, prop) {
          // @ts-expect-error -- dynamic proxy access on Theme object by property name
          // eslint-disable-next-line @typescript-eslint/no-unsafe-return -- dynamic proxy accessor returns theme color values
          return values()[prop]
        },
      }),
      get selected() {
...
function generateSyntax(theme: Theme): SyntaxStyle {
  return SyntaxStyle.fromTheme(getSyntaxRules(theme))
}

function generateSubtleSyntax(theme: Theme): SyntaxStyle {
