...
import { ArgsProvider, useArgs } from "./context/args"
...
async function getTerminalBackgroundColor(): Promise<"dark" | "light"> {
  ...
  return new Promise((resolve) => {
    // eslint-disable-next-line prefer-const -- timeout is reassigned later in setTimeout
    ...
    const handler = (data: Buffer): void => {
      ...
      // eslint-disable-next-line no-control-regex -- intentional OSC escape sequence
      ...
      if (match) {
        ...
        if (color.startsWith("rgb:")) {
          ...
          r = parseInt(parts[0] ?? "0", 16) >> 8 // Convert 16-bit to 8-bit
          g = parseInt(parts[1] ?? "0", 16) >> 8 // Convert 16-bit to 8-bit
          b = parseInt(parts[2] ?? "0", 16) >> 8 // Convert 16-bit to 8-bit
...
function App(): JSX.Element {
  ...
  renderer.console.onCopySelection = (text: string): void => {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle("Orbit")
      return
    }
    ...
    renderer.setTerminalTitle(`OB | ${title}`)
  })
  ...
  command.register(() => [
    ...
    {
      title: kv.get<boolean>("animations_enabled", true) ? "Disable animations" : "Enable animations",
      value: "app.toggle.animations",
      category: "System",
      onSelect: (dialog) => {
        kv.set("animations_enabled", !kv.get<boolean>("animations_enabled", true))
        dialog.clear()
      },
    },
    {
      title: kv.get<string>("diff_wrap_mode", "word") === "word" ? "Disable diff wrapping" : "Enable diff wrapping",
      value: "app.toggle.diffwrap",
      category: "System",
      onSelect: (dialog) => {
        const current = kv.get<string>("diff_wrap_mode", "word")
  ...
  return (
    <box
      ...
      onMouseDown={(evt) => {
        if (!Flag.OPENCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
        if ((evt.button as MouseButton) !== MouseButton.RIGHT) return
