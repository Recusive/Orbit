...
export function QuestionPrompt(props: { request: QuestionRequest }): JSX.Element {
  const sdk = useSDK()
  ...
  const question = createMemo(() => questions()[store.tab] as QuestionInfo | undefined)
  ...
  function submit(): void {
    const answers = questions().map((_q, i) => store.answers[i] ?? [])
      ...
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        <Show when={!single()}>
          <box flexDirection="row" gap={1} paddingLeft={1}>
            <For each={questions()}>
              {(q, index) => {
                const isActive = (): boolean => index() === store.tab
