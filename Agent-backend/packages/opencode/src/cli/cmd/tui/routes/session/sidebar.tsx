...
export function Sidebar(props: { sessionID: string; overlay?: boolean }): JSX.Element {
  const sync = useSync()
  const { theme } = useTheme()
  const session = createMemo(() => {
    const s = sync.session.get(props.sessionID)
    if (!s) throw new Error(`Session not found: ${props.sessionID}`)
    return s
  })
  ...
  const connectedMcpCount = createMemo(() => mcpEntries().filter(([_key, item]) => item.status === "connected").length)
