...
import type { PermissionRequest } from "@orbit.build/sdk/v2"
  ...
  return (
    <Switch>
      ...
      <Match when={store.stage === "permission"}>
        {(() => {
          // eslint-disable-next-line @typescript-eslint/explicit-function-return-type -- inferred object literal return type
          const info = () => {
...
function Prompt<const T extends Record<string, string>>(props: {
  ...
  useKeyboard((evt) => {
    ...
    if (props.escapeKey !== undefined && (evt.name === "escape" || keybind.match("app_exit", evt))) {
      evt.preventDefault()
      props.onSelect(props.escapeKey)
    }

    if (props.fullscreen === true && Keybind.match(diffKey, keybind.parse(evt))) {
      evt.preventDefault()
      evt.stopPropagation()
