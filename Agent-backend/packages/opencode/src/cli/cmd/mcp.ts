    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: orbit mcp add")
          return
        }

        for (const [name, serverConfig] of servers) {
          const status = name in statuses ? statuses[name] : undefined
          const hasOAuth = isMcpRemote(serverConfig) && serverConfig.oauth !== false && serverConfig.oauth !== undefined
        ...
        prompts.outro(`${String(servers.length)} server(s)`)
      },
    })
  },
})
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in orbit.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
  ...
  async handler() {
    await Instance.provide({
      ...
      async fn() {
        ...
        prompts.outro(`${String(oauthServers.length)} OAuth-capable server(s)`)
      },
    })
  },
})
...
function resolveConfigPath(baseDir: string, global = false): string {
  // Check for existing config files (prefer .jsonc over .json, check .orbit/ subdirectory too)
  ...
  // Default to orbit.jsonc if none exist
  return candidates[0]
}

async function addMcpToConfig(name: string, mcpConfig: Config.Mcp, configPath: string): Promise<string> {
  let text = "{}"
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., orbit x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (entry?.clientInfo) {
          ...
          if (entry.clientInfo.clientSecretExpiresAt !== undefined) {
        ...
        try {
          ...
          spinner.stop(`HTTP response: ${String(response.status)} ${response.statusText}`)
          ...
          if (response.status === 401) {
            ...
            const authProvider = new McpOAuthProvider(
              ...
              {
                onRedirect: (): void => {
                  // noop — debug mode does not complete OAuth redirect
                },
              },
            )
            ...
            try {
              const json = JSON.parse(body) as { result?: { serverInfo?: unknown } }
              if (json.result?.serverInfo !== undefined && json.result.serverInfo !== null) {
            ...
            prompts.log.warn(`Unexpected status: ${String(response.status)}`)
