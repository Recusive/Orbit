...
interface RemovalTargets {
  directories: { path: string; label: string; keep: boolean }[]
...
export const UninstallCommand = cmd({
  command: "uninstall",
  describe: "uninstall orbit and remove all related files",
  builder: (yargs) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  async handler(args) {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Orbit")

    const method = await Installation.method()
    prompts.log.info(`Installation method: ${method}`)

    const typedArgs: UninstallArgs = {
      keepConfig: args.keepConfig,
      keepData: args.keepData,
      dryRun: args.dryRun,
      force: args.force,
    }
    const targets = await collectRemovalTargets(typedArgs, method)

    await showRemovalSummary(targets, method)

    if (!typedArgs.force && !typedArgs.dryRun) {
      const confirm = await prompts.confirm({
        message: "Are you sure you want to uninstall?",
        initialValue: false,
      })
      if (confirm === false || prompts.isCancel(confirm)) {
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method): Promise<void> {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    prompts.log.info(`  ✓ Package: ${method in cmds ? cmds[method] : method}`)
  }
}

async function executeUninstall(method: Installation.Method, targets: RemovalTargets): Promise<void> {
    ...
    if (method in cmds) {
      ...
      if (result.code !== 0) {
        ...
        if (method === "choco" && text.includes("not running from an elevated command shell")) {
          prompts.log.warn(`You may need to run '${cmdArgs.join(" ")}' from an elevated command shell`)
        } else {
          prompts.log.warn(`You may need to run manually: ${cmdArgs.join(" ")}`)
        }
      } else {
  ...
  if (method === "curl" && targets.binary !== null) {
    UI.empty()
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  const candidates = shell in configFiles ? configFiles[shell] : configFiles.bash
