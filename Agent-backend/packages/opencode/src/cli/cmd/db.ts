...
const MigrateCommand = cmd({
  ...
  handler: async () => {
    ...
    try {
        ...
        progress: (event) => {
          ...
          if (tty) {
            ...
            process.stderr.write(
              `\r${orange}${bar} ${percent.toString().padStart(3)}%${reset} ${muted}${String(event.current)}/${String(event.total)}${reset} `,
            )
          } else {
            process.stderr.write(`sqlite-migration:${String(percent)}${EOL}`)
          }
        },
      })
      ...
      UI.println(
        `Migration complete: ${String(stats.projects)} projects, ${String(stats.sessions)} sessions, ${String(stats.messages)} messages`,
      )
      if (stats.errors.length > 0) {
        UI.println(`${String(stats.errors.length)} errors occurred during migration`)
      }
    } catch (err) {
      if (tty) process.stderr.write("\x1b[?25h")
