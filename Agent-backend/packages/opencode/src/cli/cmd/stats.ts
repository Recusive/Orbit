...
export const StatsCommand = cmd({
  ...
  handler: async (args) => {
    await bootstrap(process.cwd(), (): Promise<void> => {
...
export function aggregateSessionStats(days?: number, projectFilter?: string): SessionStats {
  ...
  if (filteredSessions.length > 1000) {
    console.log(`Large dataset detected (${String(filteredSessions.length)} sessions). This may take a while...`)
  }
  ...
  for (let i = 0; i < filteredSessions.length; i += BATCH_SIZE) {
    ...
    const batchPromises = batch.map((session) => {
      ...
      for (const message of messages) {
        if (message.info.role === "assistant") {
          ...
          if (!(modelKey in sessionModelUsage)) {
          ...
          sessionModelUsage[modelKey].cost += message.info.cost

          sessionTokens.input += message.info.tokens.input
          sessionTokens.output += message.info.tokens.output
          sessionTokens.reasoning += message.info.tokens.reasoning
          sessionTokens.cache.read += message.info.tokens.cache.read
        ...
        for (const part of message.parts) {
          if (part.type === "tool" && part.tool) {
            sessionToolUsage[part.tool] = (part.tool in sessionToolUsage ? sessionToolUsage[part.tool] : 0) + 1
          }
        }
      }
    ...
    for (const result of batchResults) {
      ...
      for (const [tool, count] of Object.entries(result.sessionToolUsage)) {
        stats.toolUsage[tool] = (tool in stats.toolUsage ? stats.toolUsage[tool] : 0) + count
      }

      for (const [model, usage] of Object.entries(result.sessionModelUsage)) {
        if (!(model in stats.modelUsage)) {
...
export function displayStats(stats: SessionStats, toolLimit?: number, modelLimit?: number): void {
  ...
  if (Object.keys(stats.toolUsage).length > 0) {
    ...
    const toolsToDisplay = toolLimit !== undefined && toolLimit > 0 ? sortedTools.slice(0, toolLimit) : sortedTools
