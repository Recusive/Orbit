          ...
          progress: (event) => {
            ...
            if (tty) {
              ...
              process.stderr.write(
                `\r${orange}${bar} ${String(percent).padStart(3)}%${reset} ${muted}${event.label.padEnd(12)} ${String(event.current)}/${String(event.total)}${reset}`,
              )
...
try {
  ...
  if (typeof formatted === "string") UI.error(formatted)
