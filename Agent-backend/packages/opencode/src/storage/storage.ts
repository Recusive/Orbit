...
export namespace Storage {
  ...
  const MIGRATIONS: Migration[] = [
      ...
      for (const projectDir of projectDirs) {
        ...
        if (projectID !== "global") {
          for (const msgFile of await Glob.scan("storage/session/message/*/*.json", {
            ...
            const json = await Filesystem.readJson<MigrationMessage>(msgFile)
          ...
          for (const sessionFile of await Glob.scan("storage/session/info/*.json", {
            ...
            const session = await Filesystem.readJson<MigrationSession>(sessionFile)
            ...
            for (const msgFile of await Glob.scan(`storage/session/message/${session.id}/*.json`, {
              ...
              const message = await Filesystem.readJson<MigrationMessage>(msgFile)
    ...
    async (dir) => {
      for (const item of await Glob.scan("session/*/*.json", {
        ...
        const session = await Filesystem.readJson<MigrationSession>(item)
        if (session.projectID === undefined) continue
        if (session.summary?.diffs === undefined) continue
        ...
        await Filesystem.writeJson(path.join(dir, "session", session.projectID, session.id + ".json"), {
          ...session,
          summary: {
            additions: diffs.reduce((sum: number, x) => sum + x.additions, 0),
            deletions: diffs.reduce((sum: number, x) => sum + x.deletions, 0),
          },
        })
      }
    },
  ...
  export async function update<T>(key: string[], fn: (draft: T) => void): Promise<T | undefined> {
    const dir = await state().then((x) => x.dir)
    const target = path.join(dir, ...key) + ".json"
    return withErrorHandling(async () => {
  ...
  export async function write(key: string[], content: unknown): Promise<void> {
    const dir = await state().then((x) => x.dir)
    const target = path.join(dir, ...key) + ".json"
    return withErrorHandling(async () => {
      const lock = await Lock.write(target)
  ...
  async function withErrorHandling<T>(body: () => Promise<T>): Promise<T> {
    return body().catch((e: unknown) => {
      ...
      if (errnoException.code === "ENOENT") {
        throw new NotFoundError({ message: `Resource not found: ${errnoException.path ?? "unknown"}` })
      }
      throw e
    })
  }
