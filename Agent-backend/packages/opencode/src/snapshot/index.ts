...
export namespace Snapshot {
  ...
  function args(git: string, cmd: string[]): string[] {
  ...
  export async function restore(snapshot: string): Promise<void> {
  ...
  export async function revert(patches: Patch[]): Promise<void> {
      ...
      for (const file of item.files) {
        ...
        if (result.code !== 0) {
          ...
          if (checkTree.code === 0 && checkTree.text.trim()) {
            ...
            await fs.unlink(file).catch(() => { /* ignore unlink errors */ })
          }
        }
        files.add(file)
      }
  ...
  async function add(git: string): Promise<void> {
