...
const fwd = (...parts: string[]): string => path.join(...parts).replaceAll("\\", "/")
...
/** Assert value is defined, narrowing away undefined. */
function assertDefined<T>(value: T | undefined): T {
  expect(value).toBeDefined()
  return value as T
}

async function bootstrap(): ReturnType<typeof tmpdir<{ aContent: string; bContent: string }>> {
  return tmpdir({
    git: true,
    init: async (dir) => {
...
test("tracks deleted files correctly", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const before = assertHash(await Snapshot.track())
...
test("revert with empty patches", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      await Snapshot.revert([{ hash: "dummy", files: [] }])
    },
  })
})
...
test("patch with invalid hash", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      assertHash(await Snapshot.track())
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      await fs.symlink(`${tmp.path}/circular`, `${tmp.path}/circular`, "dir").catch(() => {
        // noop — may fail if circular symlinks aren't supported
      })
...
test("git info exclude changes", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const after = assertHash(await Snapshot.track())
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const createPromise = (async () => {
        for (let i = 0; i < 10; i++) {
          await Filesystem.write(`${tmp.path}/concurrent${String(i)}.txt`, `concurrent${String(i)}`)
...
test("snapshot state isolation between projects", async () => {
  ...
  await Instance.provide({
    directory: tmp1.path,
    fn: async () => {
      const before1 = assertHash(await Snapshot.track())
  ...
  await Instance.provide({
    directory: tmp2.path,
    fn: async () => {
      const before2 = assertHash(await Snapshot.track())
...
test("track with no changes returns same hash", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const hash1 = assertHash(await Snapshot.track())
...
test("revert should not delete files that existed but were deleted in snapshot", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const snapshot2 = assertHash(await Snapshot.track())
...
test("revert preserves file that existed in snapshot when deleted then recreated", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const snapshot = assertHash(await Snapshot.track())
...
test("diffFull sets status based on git change type", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const added = assertDefined(diffs.find((d) => d.file === "added.txt"))
      expect(added.status).toBe("added")

      const deleted = assertDefined(diffs.find((d) => d.file === "delete.txt"))
      expect(deleted.status).toBe("deleted")

      const grow = assertDefined(diffs.find((d) => d.file === "grow.txt"))
      expect(grow.status).toBe("modified")
      expect(grow.additions).toBeGreaterThan(0)
      expect(grow.deletions).toBe(0)

      const trim = assertDefined(diffs.find((d) => d.file === "trim.txt"))
...
test("diffFull with addition and deletion", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const addedFileDiff = assertDefined(diffs.find((d) => d.file === "added.txt"))
      ...
      const removedFileDiff = assertDefined(diffs.find((d) => d.file === "a.txt"))
      expect(removedFileDiff.before).toBe(tmp.extra.aContent)
      expect(removedFileDiff.after).toBe("")
      expect(removedFileDiff.additions).toBe(0)
      expect(removedFileDiff.deletions).toBe(1)
...
test("diffFull with multiple additions and deletions", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const multi1Diff = assertDefined(diffs.find((d) => d.file === "multi1.txt"))
      expect(multi1Diff.additions).toBe(3)
      expect(multi1Diff.deletions).toBe(0)

      const multi2Diff = assertDefined(diffs.find((d) => d.file === "multi2.txt"))
      expect(multi2Diff.additions).toBe(1)
      expect(multi2Diff.deletions).toBe(0)

      const removedADiff = assertDefined(diffs.find((d) => d.file === "a.txt"))
      expect(removedADiff.additions).toBe(0)
      expect(removedADiff.deletions).toBe(1)

      const removedBDiff = assertDefined(diffs.find((d) => d.file === "b.txt"))
