...
test("legacy opencode provider config is normalized to orbit", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "orbit.json"),
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const providers = await Provider.list()
      expect(providers.orbit).toBeDefined()
      expect(providers.orbit.options.apiKey).toBe("config-api-key")
      expect(Provider.parseModel("opencode/big-pickle")).toEqual({
        providerID: "orbit",
        modelID: "big-pickle",
      })
    },
