...
beforeAll(async () => {
  ...
  CLOUDFLARE_SKILLS_URL = `http://localhost:${String(server.port)}/.well-known/skills/`
})
...
describe("Discovery.pull", () => {
  test("downloads skills from cloudflare url", async () => {
    ...
    for (const dir of dirs) {
      ...
      expect(Filesystem.exists(md)).toBe(true)
    }
  })
  ...
  test("returns empty array for invalid url", async () => {
    const dirs = await Discovery.pull(`http://localhost:${String(server.port)}/invalid-url/`)
    expect(dirs).toEqual([])
  })

  test("returns empty array for non-json response", async () => {
    // any url not explicitly handled in server returns 404 text "Not Found"
    const dirs = await Discovery.pull(`http://localhost:${String(server.port)}/some-other-path/`)
    expect(dirs).toEqual([])
  })
  ...
  test("downloads reference files alongside SKILL.md", async () => {
    ...
    if (agentsSdk) {
      ...
      expect(Filesystem.exists(path.join(agentsSdk, "SKILL.md"))).toBe(true)
