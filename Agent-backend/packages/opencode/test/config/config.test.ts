...
async function writeConfig(dir: string, config: object, name = "orbit.json"): Promise<void> {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

async function check(map: (dir: string) => string): Promise<void> {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "orbit.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "orbit.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .orbit directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".orbit")
...
test("loads agents from .orbit/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".orbit")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .orbit/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".orbit")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .orbit/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".orbit")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("installs dependencies in writable OPENCODE_CONFIG_DIR", async () => {
  ...
  try {
    ...
    expect(Filesystem.exists(path.join(tmp.extra, "package.json"))).toBe(true)
    expect(Filesystem.exists(path.join(tmp.extra, ".gitignore"))).toBe(true)
  } finally {
    if (prev === undefined) delete process.env.OPENCODE_CONFIG_DIR
    else process.env.OPENCODE_CONFIG_DIR = prev
  }
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .orbit config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".orbit")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .orbit config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "orbit.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .orbit config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "orbit.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("local .orbit config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .orbit directory config enables it
      const opencodeDir = path.join(dir, ".orbit")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "orbit.json"),
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.orbit/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.orbit/plugin/oh-my-opencode.js")
  })
  ...
  test("local plugin directory overrides global orbit.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const opencodeDir = path.join(projectDir, ".orbit")
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .orbit/ directories when flag is set", async () => {
    const originalEnv = process.env.OPENCODE_DISABLE_PROJECT_CONFIG
    process.env.OPENCODE_DISABLE_PROJECT_CONFIG = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .orbit directory with a command
          const opencodeDir = path.join(dir, ".orbit", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .orbit should NOT be in directories list
