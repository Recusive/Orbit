...
test("migrates tui-specific keys from orbit.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "orbit.json")))
      expect(server.theme).toBeUndefined()
      expect(server.keybinds).toBeUndefined()
      expect(server.tui).toBeUndefined()
      expect(Filesystem.exists(path.join(tmp.path, "orbit.json.tui-migration.bak"))).toBe(true)
      expect(Filesystem.exists(path.join(tmp.path, "tui.json"))).toBe(true)
    },
  })
})
...
test("skips migration when orbit.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "orbit.jsonc"),
        `{
  "theme": "broken-theme",
  "tui": { "scroll_speed": 2 }
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(Filesystem.exists(path.join(tmp.path, "tui.json"))).toBe(false)
      expect(Filesystem.exists(path.join(tmp.path, "orbit.jsonc.tui-migration.bak"))).toBe(false)
      const source = await Filesystem.readText(path.join(tmp.path, "orbit.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "orbit.json"), JSON.stringify({ theme: "legacy" }, null, 2))
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(Filesystem.exists(path.join(tmp.path, "orbit.json.tui-migration.bak"))).toBe(false)
    },
  })
})
...
test("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "orbit.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "orbit.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await TuiConfig.get()
      const backup = await Filesystem.readText(path.join(tmp.path, "orbit.jsonc.tui-migration.bak"))
...
test("migrates legacy tui keys across multiple orbit.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await Bun.write(path.join(dir, "orbit.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "orbit.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(Filesystem.exists(path.join(tmp.path, "tui.json"))).toBe(true)
      expect(Filesystem.exists(path.join(tmp.path, "apps", "client", "tui.json"))).toBe(true)
    },
  })
})
...
test("does not derive tui path from OPENCODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(customDir, "orbit.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.OPENCODE_CONFIG = path.join(customDir, "orbit.json")
    },
  })
...
test("loads .orbit/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".orbit"), { recursive: true })
      await Bun.write(path.join(dir, ".orbit", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
