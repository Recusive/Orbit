...
const dir = path.join(os.tmpdir(), "opencode-test-data-" + String(process.pid))
...
afterAll(async () => {
  ...
  const rm = async (left: number): Promise<void> => {
    Bun.gc(true)
    await sleep(100)
    return fs.rm(dir, { recursive: true, force: true }).catch((error: unknown) => {
...
const cacheDir = path.join(dir, "cache", "orbit")
