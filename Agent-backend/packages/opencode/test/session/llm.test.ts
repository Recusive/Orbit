...
function waitRequest(pathname: string, response: Response): Promise<Capture> {
...
function createChatStream(text: string): ReadableStream<Uint8Array> {
  const payload =
    [
      `data: ${JSON.stringify({
        id: "chatcmpl-1",
...
async function loadFixture(providerID: string, modelID: string): Promise<{ provider: ModelsDev.Provider; model: ModelsDev.Model }> {
  ...
  const model = provider.models[modelID] as ModelsDev.Model | undefined
  if (model === undefined) {
...
function createEventStream(chunks: unknown[], includeDone = false): ReadableStream<Uint8Array> {
...
function createEventResponse(chunks: unknown[], includeDone = false): Response {
