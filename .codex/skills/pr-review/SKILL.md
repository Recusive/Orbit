---
name: pr-review
description: Run a senior-level code review of branch changes vs main. Use when asked to "review this PR", "code review", "review my changes", or "check my branch".
...
**Cycle**: $ARGUMENTS[0] (first argument)
**Model**: $ARGUMENTS[1] (second argument)
**Output**: `reviews/cycle-{cycle}-{model}.md`

## Validation

If cycle number or model name is missing, output:

```
Usage: pr-review <cycle-number> <model-name>
Example: pr-review 1 codex
```
...
First, run these commands to get context:
...
Get the current date and branch:
...
Create `reviews/cycle-{cycle}-{model}.md`:
...
**Branch**: [from git branch command above]
...
1. Print: `Review written to reviews/cycle-{cycle}-{model}.md`
