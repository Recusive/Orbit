...
/// Convert snowflake-fs FileEvent to our FileChangeEvent for the frontend.
fn convert_file_event(event: snowflake_fs::FileEvent) -> Option<FileChangeEvent> {
    match event {
        snowflake_fs::FileEvent::Created { path } => Some(FileChangeEvent::Created { path }),
        snowflake_fs::FileEvent::Modified { path } => Some(FileChangeEvent::Modified { path }),
        snowflake_fs::FileEvent::Deleted { path } => Some(FileChangeEvent::Deleted { path }),
        snowflake_fs::FileEvent::Renamed { from, to } => Some(FileChangeEvent::Renamed {
            path: from,
            new_path: to,
        }),
