...
for CYCLE in $(seq 1 $CYCLES); do
    ...
    if [ "$CYCLE" -eq 1 ]; then
        ...
        PROMPT="Run /nightshift on this codebase. You are already inside the nightshift worktree — do NOT create a new worktree or switch branches. This is the FINAL cycle (${CYCLE}/${CYCLES}) of a test run. Read the existing shift log at ${SHIFT_LOG} to see what's done. Find and fix 2-3 MORE issues — pick different files and strategies than what's already logged. Then wrap up the full shift: write the Summary paragraph, add Recommendations, update final stats, run the test suite, and make a final commit."
    else
