...
interface UseQueuedMessageReturn {
  ...
  queueMessage: (text: string, contextFiles?: string[], images?: ImageAttachment[], elements?: ReactElementContext[]) => void;
  cancelQueue: () => void;
}
...
export function useQueuedMessageHandler(options: UseQueuedMessageOptions): UseQueuedMessageReturn {
  ...
  }, [isAgentRunning, queuedMessage, sessionId, postMessage, addMessage, setIsAgentRunning, clearQueue]);

  const queueMessage = (text: string, contextFiles?: string[], images?: ImageAttachment[], elements?: ReactElementContext[]): void => {
