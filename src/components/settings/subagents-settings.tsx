      ...
      <DialogPrimitive.Portal>
        ...
        >
          ...
          <DialogPrimitive.Description className="sr-only">
            {agent !== undefined ? 'Edit an existing subagent configuration' : 'Create a new subagent configuration'}
          ...
          <ScrollArea className="flex-1 py-4 px-5">
            <div className="space-y-4 px-px">
              ...
              <div>
                ...
                />
                <p className="text-[10px] text-muted-foreground mt-1">
                  Describes when this agent should be used. Claude uses this to decide when to invoke it.
                </p>
              </div>
...
export const SubagentsSettings: FC = () => {
  ...
  const handleMessage = useCallback((message: ExtensionMessage): void => {
    if (message.type === 'subagents:list:response') {
      ...
      setAgents((prev) =>
        prev.map((a) => (a.name === message.agent.name ? message.agent : a))
      );
      setError(null);
  ...
  return (
    <div>
      <SectionHeader title="Subagents">
        Custom agents that can be invoked via the Task tool for specialized tasks.
        Subagents maintain separate context and can run in parallel.
      </SectionHeader>
      ...
      <div className="mt-6 p-3 rounded-md bg-muted/50 text-xs text-muted-foreground">
        ...
        <ul className="list-disc list-inside space-y-0.5">
          <li>Subagents are stored in <code className="bg-muted px-1 rounded">.claude/agents/</code></li>
