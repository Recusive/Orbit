...
const CommandCard: FC<CommandCardProps> = ({ command, onEdit, onDelete }) => {
  const isReadonly = command.readonly === true || command.scope === 'builtin' || command.scope === 'default';
      ...
      <DialogPrimitive.Portal>
        ...
        >
          ...
          <DialogPrimitive.Description className="sr-only">
            {command !== undefined ? 'Edit an existing slash command' : 'Create a new slash command'}
          ...
          <ScrollArea className="flex-1 py-4 px-5">
            <div className="space-y-4 px-px">
              ...
              <div>
                ...
                />
                <p className="text-[10px] text-muted-foreground mt-1">
                  Shows users what arguments this command accepts, e.g., &quot;[file] [options]&quot;
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p className="text-[10px] text-muted-foreground mt-1">
                  Use <code className="bg-muted px-1 rounded">$ARGUMENTS</code> for user input.
...
export const SlashCommandsSettings: FC = () => {
  ...
  const [editingCommand, setEditingCommand] = useState<SlashCommandDefinition | undefined>(undefined);
  ...
  const handleMessage = useCallback((message: ExtensionMessage): void => {
    if (message.type === 'commands:list:response') {
      ...
      setCommands((prev) =>
        prev.map((c) => (c.name === message.command.name && c.scope === message.command.scope ? message.command : c))
      );
      setError(null);
    ...
    <div>
                ...
                <div className="space-y-2">
                  {personalCommands.map((command) => (
                    <CommandCard
                      ...
                      onDelete={() => { handleDeleteCommand(command.name, 'personal'); }}
                    />
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {projectCommands.map((command) => (
                    <CommandCard
                      ...
                      command={command}
                      onEdit={() => { handleEditCommand(command); }}
                      onDelete={() => { handleDeleteCommand(command.name, 'project'); }}
                    />
                  ))}
                </div>
              </div>
            ...
            {commands.length === 0 && (
              <div className="text-center py-8 text-muted-foreground text-sm">
                ...
                <p className="text-xs mt-1">
                  Create a command to quickly run common prompts.
                </p>
              </div>
            )}
          </div>
      ...
      <div className="mt-6 p-3 rounded-md bg-muted/50 text-xs text-muted-foreground">
        ...
        <ul className="list-disc list-inside space-y-0.5">
          <li>Project commands are stored in <code className="bg-muted px-1 rounded">.claude/commands/</code></li>
          <li>Personal commands are stored in <code className="bg-muted px-1 rounded">~/.claude/commands/</code></li>
          <li>Type <code className="bg-muted px-1 rounded">/command-name</code> in chat to run</li>
          <li>Use <code className="bg-muted px-1 rounded">$ARGUMENTS</code> to pass user input</li>
          <li>Use <code className="bg-muted px-1 rounded">@filename</code> to include file contents</li>
        </ul>
      </div>
