...
/// AppleScript-facing wrapper around a live OrbitTerminal terminal surface.
///
...
///
/// Mapping from `OrbitTerminal.sdef`:
/// - `class terminal` -> this class (`@objc(OrbitTerminalAppleScriptTerminal)`).
...
@MainActor
@objc(OrbitTerminalScriptTerminal)
final class ScriptTerminal: NSObject {
    ...
    weak var surfaceView: OrbitTerminal.SurfaceView?

    init(surfaceView: OrbitTerminal.SurfaceView) {
...
/// Converts four-character codes from the `split direction` enumeration in `OrbitTerminal.sdef`
...
enum ScriptSplitDirection {
    ...
    var splitDirection: SplitTree<OrbitTerminal.SurfaceView>.NewDirection {
        switch self {
        case .right: .right
        case .left: .left
        case .down: .down
