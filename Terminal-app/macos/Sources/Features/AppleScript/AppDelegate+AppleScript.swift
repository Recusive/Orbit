...
// Application-level Cocoa scripting hooks for the OrbitTerminal AppleScript dictionary.
//
...
// the runtime object behind the `application` class in `OrbitTerminal.sdef`.
...
extension NSApplication {
    ...
    /// the frontmost logical OrbitTerminal window.
    @objc(frontWindow)
...
extension NSApplication {
    ...
    func handleNewSurfaceConfigurationScriptCommand(_ command: NSScriptCommand) -> NSDictionary? {
        ...
        do {
            let configuration = try OrbitTerminal.SurfaceConfiguration(
    ...
    func handleNewWindowScriptCommand(_ command: NSScriptCommand) -> ScriptWindow? {
        ...
        guard let appDelegate = delegate as? AppDelegate else {
            ...
            command.scriptErrorString = "OrbitTerminal app delegate is unavailable."
            return nil
        }

        let baseConfig: OrbitTerminal.SurfaceConfiguration?
        if let scriptRecord = command.evaluatedArguments?["configuration"] as? NSDictionary {
            do {
                baseConfig = try OrbitTerminal.SurfaceConfiguration(scriptRecord: scriptRecord)
            } catch {
                command.scriptErrorNumber = errAECoercionFail
                command.scriptErrorString = error.localizedDescription
                return nil
...
@MainActor
extension NSApplication {
    /// Whether OrbitTerminal should currently accept AppleScript interactions.
    ...
    fileprivate var allSurfaceViews: [OrbitTerminal.SurfaceView] {
