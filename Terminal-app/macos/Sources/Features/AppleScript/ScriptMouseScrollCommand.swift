...
/// Handler for the `send mouse scroll` AppleScript command defined in `OrbitTerminal.sdef`.
///
/// Cocoa scripting instantiates this class because the command's `<cocoa>` element
/// specifies `class="OrbitTerminalScriptMouseScrollCommand"`. The runtime calls
/// `performDefaultImplementation()` to execute the command.
@MainActor
@objc(OrbitTerminalScriptMouseScrollCommand)
final class ScriptMouseScrollCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        let momentum: OrbitTerminal.Input.Momentum
