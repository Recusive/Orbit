...
enum TerminalSplitOperation {
    ...
    struct Resize {
        let node: SplitTree<OrbitTerminal.SurfaceView>.Node
        let ratio: Double
    }

    struct Drop {
        /// The surface being dragged.
        let payload: OrbitTerminal.SurfaceView

        /// The surface it was dragged onto
        let destination: OrbitTerminal.SurfaceView
...
struct TerminalSplitTreeView: View {
    let tree: SplitTree<OrbitTerminal.SurfaceView>
...
private struct TerminalSplitSubtreeView: View {
    @EnvironmentObject var ghostty: OrbitTerminal.App
...
private struct TerminalSplitLeaf: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            OrbitTerminal.InspectableSurface(
            ...
            .background {
                ...
                if !isSelfDragging {
                    Color.clear
                        .onDrop(of: [.orbitTerminalSurfaceId], delegate: SplitDropDelegate(
                            dropState: $dropState,
            ...
            .onPreferenceChange(OrbitTerminal.DraggingSurfaceKey.self) { value in
    ...
    private struct SplitDropDelegate: DropDelegate {
        ...
        let viewSize: CGSize
        let destinationSurface: OrbitTerminal.SurfaceView
        let action: (TerminalSplitOperation) -> Void

        func validateDrop(info: DropInfo) -> Bool {
            info.hasItemsConforming(to: [.orbitTerminalSurfaceId])
        }
        ...
        func performDrop(info: DropInfo) -> Bool {
            ...
            let providers = info.itemProviders(for: [.orbitTerminalSurfaceId])
            guard let provider = providers.first else { return false }

            // Capture action before the async closure
            _ = provider.loadTransferable(type: OrbitTerminal.SurfaceView.self) { [weak destinationSurface] result in
                switch result {
