    ...
    private func setupObservers() {
        ...
        if bellTriggersAttention {
            let bellObserver = center.addObserver(
                forName: .orbitTerminalBellDidRing,
                object: nil,
                queue: .main
                ...
                // Bell notifications mark the window for attention
        ...
        let desktopNotifObserver = center.addObserver(
            forName: .orbitTerminalDesktopNotificationDidFire,
            object: nil,
            queue: .main
        ) { [weak self] notification in
            guard let self,
                  let surfaceView = notification.object as? OrbitTerminal.SurfaceView,
        ...
        let ipcNotifObserver = center.addObserver(
            forName: .orbitTerminalIPCNotification,
            object: nil,
            queue: .main
        ) { [weak self] notification in
            guard let self,
