...
/// A Unix domain socket server that allows external processes to control Orbit Terminal tabs.
///
/// Protocol: newline-delimited JSON over a Unix domain socket at `/tmp/orbit-terminal-{uid}.sock`.
///
/// Request format:
/// ```json
...
/// ```
@MainActor
final class OrbitTerminalIPCServer {
    static let shared = OrbitTerminalIPCServer()

    private static let logger = Logger(
        subsystem: Bundle.main.bundleIdentifier ?? "com.orbit.orbit-terminal",
        category: "IPC"
    )
    ...
    func start() {
        let uid = getuid()
        socketPath = "/tmp/orbit-terminal-\(uid).sock"
    ...
    private func handleTabNotify(params: [String: Any], client: ClientConnection) {
        let title = params["title"] as? String ?? "Orbit Terminal"
        ...
        if let controller = resolveController(params: params),
           let window = controller.window {
            NotificationCenter.default.post(
                name: .orbitTerminalIPCNotification,
                object: window
            )
        }
    ...
    /// Resolve an `OrbitTerminal.SurfaceView` from params.
    private func resolveSurface(params: [String: Any]) -> OrbitTerminal.SurfaceView? {
        if let tabIdStr = params["tab_id"] as? String,
           let tabId = UUID(uuidString: tabIdStr) {
    ...
    private func surfaceForId(_ id: UUID) -> OrbitTerminal.SurfaceView? {
        for window in NSApp.windows {
            guard let controller = window.windowController as? BaseTerminalController else { continue }
    ...
    private func tabInfo(
        surface: OrbitTerminal.SurfaceView,
