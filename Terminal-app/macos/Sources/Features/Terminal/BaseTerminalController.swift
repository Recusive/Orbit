...
/// A base class for windows that can contain OrbitTerminal windows. This base class implements
/// the bare minimum functionality that every terminal window in OrbitTerminal should implement.
///
...
class BaseTerminalController: NSWindowController,
    ...
    var focusedSurface: OrbitTerminal.SurfaceView? {
        didSet { syncFocusToSurfaceTree() }
    }

    /// The tree of splits within this terminal window.
    @Published var surfaceTree: SplitTree<OrbitTerminal.SurfaceView> = .init() {
    ...
    init(_ ghostty: OrbitTerminal.App,
         ...
         surfaceTree tree: SplitTree<OrbitTerminal.SurfaceView>? = nil
    ) {
        ...
        center.addObserver(
            ...
            name: OrbitTerminal.Notification.confirmClipboard,
            object: nil)
        center.addObserver(
            self,
        ...
        center.addObserver(
            self,
            selector: #selector(orbitTerminalConfigDidChangeBase(_:)),
            name: .orbitTerminalConfigDidChange,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(orbitTerminalCommandPaletteDidToggle(_:)),
            name: .orbitTerminalCommandPaletteDidToggle,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(orbitTerminalMaximizeDidToggle(_:)),
            name: .orbitTerminalMaximizeDidToggle,
            object: nil)
        ...
        center.addObserver(
            ...
            name: OrbitTerminal.Notification.orbitTerminalCloseSurface,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(ghosttyDidNewSplit(_:)),
            name: OrbitTerminal.Notification.orbitTerminalNewSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(ghosttyDidEqualizeSplits(_:)),
            name: OrbitTerminal.Notification.didEqualizeSplits,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(ghosttyDidFocusSplit(_:)),
            name: OrbitTerminal.Notification.orbitTerminalFocusSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(ghosttyDidToggleSplitZoom(_:)),
            name: OrbitTerminal.Notification.didToggleSplitZoom,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(ghosttyDidResizeSplit(_:)),
            name: OrbitTerminal.Notification.didResizeSplit,
            object: nil)
        center.addObserver(
            self,
    ...
    @discardableResult
    func newSplit(
        at oldView: OrbitTerminal.SurfaceView,
        direction: SplitTree<OrbitTerminal.SurfaceView>.NewDirection,
        baseConfig config: OrbitTerminal.SurfaceConfiguration? = nil
    ) -> OrbitTerminal.SurfaceView? {
        ...
        let newView = OrbitTerminal.SurfaceView(ghostty_app, baseConfig: config)

        // Do the split
        let newTree: SplitTree<OrbitTerminal.SurfaceView>
        do {
            ...
            // no big deal.
            OrbitTerminal.logger.warning("failed to insert split: \(error)")
            return nil
        }
    ...
    func focusSurface(_ view: OrbitTerminal.SurfaceView) {
    ...
    func surfaceTreeDidChange(from: SplitTree<OrbitTerminal.SurfaceView>, to: SplitTree<OrbitTerminal.SurfaceView>) {
    ...
    func closeSurface(
        _ view: OrbitTerminal.SurfaceView,
    ...
    func closeSurface(
        ...
        // due to SwiftUI bugs (see OrbitTerminal #560). To repeat from #560, the bug is that
    ...
    private func findNextFocusTargetAfterClosing(node: SplitTree<OrbitTerminal.SurfaceView>.Node) -> OrbitTerminal.SurfaceView? {
    ...
    private func removeSurfaceNode(_ node: SplitTree<OrbitTerminal.SurfaceView>.Node) {
        // Move focus if the closed surface was focused and we have a next target
        let nextFocus: OrbitTerminal.SurfaceView? = if node.contains(
    ...
    func replaceSurfaceTree(
        ...
        if let newView {
            DispatchQueue.main.async {
                OrbitTerminal.moveFocus(to: newView, from: oldView)
            }
        }
        ...
        undoManager.registerUndo(
            ...
            if let oldView {
                DispatchQueue.main.async {
                    OrbitTerminal.moveFocus(to: oldView, from: target.focusedSurface)
                }
            }
    ...
    @objc private func orbitTerminalConfigDidChangeBase(_ notification: Notification) {
    ...
    @objc private func orbitTerminalCommandPaletteDidToggle(_ notification: Notification) {
        guard let surfaceView = notification.object as? OrbitTerminal.SurfaceView else { return }
        guard surfaceTree.contains(surfaceView) else { return }
        toggleCommandPalette(nil)
    }

    @objc private func orbitTerminalMaximizeDidToggle(_ notification: Notification) {
        guard let window else { return }
        guard let surfaceView = notification.object as? OrbitTerminal.SurfaceView else { return }
        guard surfaceTree.contains(surfaceView) else { return }
        window.zoom(nil)
    ...
    @objc private func ghosttyDidNewSplit(_ notification: Notification) {
        ...
        guard let oldView = notification.object as? OrbitTerminal.SurfaceView else { return }
        ...
        let splitDirection: SplitTree<OrbitTerminal.SurfaceView>.NewDirection
        switch direction {
        case ORBIT_TERMINAL_SPLIT_DIRECTION_RIGHT: splitDirection = .right
        case ORBIT_TERMINAL_SPLIT_DIRECTION_LEFT: splitDirection = .left
        case ORBIT_TERMINAL_SPLIT_DIRECTION_DOWN: splitDirection = .down
        case ORBIT_TERMINAL_SPLIT_DIRECTION_UP: splitDirection = .up
        default: return
        }
    ...
    @objc private func ghosttyDidFocusSplit(_ notification: Notification) {
        ...
        guard let directionAny = notification.userInfo?[OrbitTerminal.Notification.SplitDirectionKey] else { return }
        guard let direction = directionAny as? OrbitTerminal.SplitFocusDirection else { return }
        ...
        DispatchQueue.main.async {
            OrbitTerminal.moveFocus(to: nextSurface, from: target)
        }
    }
    ...
    @objc private func ghosttyDidToggleSplitZoom(_ notification: Notification) {
        ...
        DispatchQueue.main.async {
            OrbitTerminal.moveFocus(to: target)
        }
    }
    ...
    @objc private func ghosttyDidResizeSplit(_ notification: Notification) {
        ...
        guard let directionAny = notification.userInfo?[OrbitTerminal.Notification.ResizeSplitDirectionKey] else { return }
        guard let direction = directionAny as? OrbitTerminal.SplitResizeDirection else { return }

        guard let amountAny = notification.userInfo?[OrbitTerminal.Notification.ResizeSplitAmountKey] else { return }
        guard let amount = amountAny as? UInt16 else { return }

        // Convert OrbitTerminal.SplitResizeDirection to SplitTree.Spatial.Direction
        let spatialDirection: SplitTree<OrbitTerminal.SurfaceView>.Spatial.Direction
        switch direction {
        ...
        do {
            ...
            OrbitTerminal.logger.warning("failed to resize split: \(error)")
        }
    }
    ...
    @objc private func ghosttyDidPresentTerminal(_ notification: Notification) {
        ...
        OrbitTerminal.moveFocus(to: target)
        OrbitTerminal.moveFocus(to: target, delay: 0.1)
    ...
    @objc private func ghosttySurfaceDragEndedNoTarget(_ notification: Notification) {
        ...
        let newTree = SplitTree<OrbitTerminal.SurfaceView>(view: target)
    ...
    private func localEventFlagsChanged(_ event: NSEvent) -> NSEvent? {
        var surfaces: [OrbitTerminal.SurfaceView] = surfaceTree.map { $0 }
    ...
    private func splitDidResize(node: SplitTree<OrbitTerminal.SurfaceView>.Node, to newRatio: Double) {
        let resizedNode = node.resizing(to: newRatio)
        do {
            surfaceTree = try surfaceTree.replacing(node: node, with: resizedNode)
        } catch {
            OrbitTerminal.logger.warning("failed to replace node during split resize: \(error)")
        }
    }

    private func splitDidDrop(
        source: OrbitTerminal.SurfaceView,
        destination: OrbitTerminal.SurfaceView,
        zone: TerminalSplitDropZone
    ) {
        // Map drop zone to split direction
        let direction: SplitTree<OrbitTerminal.SurfaceView>.NewDirection = switch zone {
        case .top: .up
        case .bottom: .down
        case .left: .left
        case .right: .right
        ...
        if let sourceNode = surfaceTree.root?.node(view: source) {
            ...
            do {
                ...
                OrbitTerminal.logger.warning("failed to insert surface during drop: \(error)")
                return
            }
        ...
        var sourceNode: SplitTree<OrbitTerminal.SurfaceView>.Node?
        ...
        guard let sourceController, let sourceNode else {
            OrbitTerminal.logger.warning("source surface not found in any window during drop")
            return
        }
        ...
        do {
            ...
            OrbitTerminal.logger.warning("failed to insert surface during cross-window drop: \(error)")
            return
        }
    ...
    func performAction(_ action: String, on surfaceView: OrbitTerminal.SurfaceView) {
    ...
    @objc private func onConfirmClipboardRequest(notification: SwiftUI.Notification) {
        ...
        guard let str = notification.userInfo?[OrbitTerminal.Notification.ConfirmClipboardStrKey] as? String else { return }
        guard let state = notification.userInfo?[OrbitTerminal.Notification.ConfirmClipboardStateKey] as? UnsafeMutableRawPointer? else { return }
        guard let request = notification.userInfo?[OrbitTerminal.Notification.ConfirmClipboardRequestKey] as? OrbitTerminal.ClipboardRequest else { return }

        // If we already have a clipboard confirmation view up, we ignore this request.
        // This shouldn't be possible...
        guard self.clipboardConfirmation == nil else {
            OrbitTerminal.App.completeClipboardRequest(surface, data: "", state: state, confirmed: true)
            return
        }
    ...
    func clipboardConfirmationComplete(_ action: ClipboardConfirmationView.Action, _ request: OrbitTerminal.ClipboardRequest) {
        ...
        case .osc_52_read, .paste:
            ...
            OrbitTerminal.App.completeClipboardRequest(cc.surface, data: str, state: cc.state, confirmed: true)
        }
    }
    ...
    func windowDidBecomeKey(_ notification: Notification) {
        ...
        if let window, window.firstResponder == window, let focusedSurface {
            DispatchQueue.main.async {
                OrbitTerminal.moveFocus(to: focusedSurface)
            }
        }
    ...
    @IBAction func splitRight(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        ghostty.split(surface: surface, direction: ORBIT_TERMINAL_SPLIT_DIRECTION_RIGHT)
    }

    @IBAction func splitLeft(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        ghostty.split(surface: surface, direction: ORBIT_TERMINAL_SPLIT_DIRECTION_LEFT)
    }

    @IBAction func splitDown(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        ghostty.split(surface: surface, direction: ORBIT_TERMINAL_SPLIT_DIRECTION_DOWN)
    }

    @IBAction func splitUp(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        ghostty.split(surface: surface, direction: ORBIT_TERMINAL_SPLIT_DIRECTION_UP)
    }
    ...
    private func splitMoveFocus(direction: OrbitTerminal.SplitFocusDirection) {
        guard let surface = focusedSurface?.surface else { return }
    ...
    private struct DerivedConfig {
        let macosTitlebarProxyIcon: OrbitTerminal.MacOSTitlebarProxyIcon
        let windowStepResize: Bool
        let focusFollowsMouse: Bool
        let splitPreserveZoom: OrbitTerminal.Config.SplitPreserveZoom
...
extension BaseTerminalController {
    ...
    func surfaceValuesPublisher<Value>(
        valueKeyPath: KeyPath<OrbitTerminal.SurfaceView, Value>,
        publisherKeyPath: KeyPath<OrbitTerminal.SurfaceView, Published<Value>.Publisher>
    ) -> AnyPublisher<[OrbitTerminal.SurfaceView.ID: Value], Never> {
...
extension Notification.Name {
    ...
    static let terminalWindowBellDidChangeNotification = Notification.Name("com.orbit.orbit-terminal.terminalWindowBellDidChange")
