...
extension NSWindow {
    ...
    func addTabbedWindowSafely(
        ...
        var error: NSError?
        let success = OrbitTerminalAddTabbedWindowSafely(self, child, ordered.rawValue, &error)
        if let error {
            OrbitTerminal.logger.error("addTabbedWindow failed: \(error.localizedDescription)")
        }
