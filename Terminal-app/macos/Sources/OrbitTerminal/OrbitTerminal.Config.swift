...
extension OrbitTerminal {
    ...
    class Config: ObservableObject {
        // The underlying C pointer to the OrbitTerminal config structure. This
        ...
        /// in the OrbitTerminal configuration. For example `keybind = cmd+q=quit` in OrbitTerminal
        ...
        func keyboardShortcut(for action: String) -> KeyboardShortcut? {
            ...
            return OrbitTerminal.keyboardShortcut(for: trigger)
        }
#endif

        // MARK: - Configuration Values

        /// For all of the configuration values below, see the associated OrbitTerminal documentation for
        ...
        var macosCustomIcon: String {
            #if os(macOS)
            let defaultValue = NSString("~/.config/ghostty/OrbitTerminal.icns").expandingTildeInPath
            guard let config = self.config else { return defaultValue }
            var v: UnsafePointer<Int8>?
...
#error("unsupported")
        ...
        var quickTerminalScreen: QuickTerminalScreen {
            ...
            let str = String(cString: ptr)
            return QuickTerminalScreen(fromOrbitTerminalConfig: str) ?? .main
        }
        ...
        var quickTerminalSpaceBehavior: QuickTerminalSpaceBehavior {
            ...
            let str = String(cString: ptr)
            return QuickTerminalSpaceBehavior(fromOrbitTerminalConfig: str) ?? .move
        }
        ...
        var commandPaletteEntries: [OrbitTerminal.Command] {
            guard let config = self.config else { return [] }
            ...
            return buffer.map { OrbitTerminal.Command(cValue: $0) }
        }
...
extension OrbitTerminal.Config {
