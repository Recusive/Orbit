...
extension OrbitTerminal {
    ...
    class SurfaceView: OSView, ObservableObject, Codable, Identifiable {
        ...
        // detected with the set_password_input_cb on the OrbitTerminal state.
        ...
        var inspector: OrbitTerminal.Inspector? {
            guard let surface = self.surface else { return nil }
            guard let cInspector = ghostty_surface_inspector(surface) else { return nil }
            return OrbitTerminal.Inspector(cInspector: cInspector)
        }
        ...
        private(set) var surfaceModel: OrbitTerminal.Surface?
        ...
        var scrollbar: OrbitTerminal.Action.Scrollbar?
        ...
        init(_ app: ghostty_app_t, baseConfig: SurfaceConfiguration? = nil, uuid: UUID? = nil) {
            ...
            center.addObserver(
                ...
                name: OrbitTerminal.Notification.didUpdateRendererHealth,
                object: self)
            center.addObserver(
                self,
                selector: #selector(ghosttyDidContinueKeySequence),
                name: OrbitTerminal.Notification.didContinueKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(ghosttyDidEndKeySequence),
                name: OrbitTerminal.Notification.didEndKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(ghosttyDidChangeKeyTable),
                name: OrbitTerminal.Notification.didChangeKeyTable,
                object: self)
            center.addObserver(
                self,
            ...
            center.addObserver(
                self,
                selector: #selector(orbitTerminalColorDidChange(_:)),
                name: .orbitTerminalColorDidChange,
                object: self)
            center.addObserver(
                self,
            ...
            center.addObserver(
                self,
                selector: #selector(orbitTerminalDidChangeReadonly(_:)),
            ...
            guard let surface = surface else {
                self.error = OrbitTerminal.Error.apiFailed
                return
            }
            self.surfaceModel = OrbitTerminal.Surface(cSurface: surface)
        ...
        func sizeDidChange(_ size: CGSize) {
            // OrbitTerminal wants to know the actual framebuffer size... It is very important
        ...
        @objc private func ghosttyDidContinueKeySequence(notification: SwiftUI.Notification) {
            guard let keyAny = notification.userInfo?[OrbitTerminal.Notification.KeySequenceKey] else { return }
        ...
        @objc private func ghosttyDidChangeKeyTable(notification: SwiftUI.Notification) {
            guard let action = notification.userInfo?[OrbitTerminal.Notification.KeyTableKey] as? OrbitTerminal.Action.KeyTable else { return }
        ...
        @objc private func orbitTerminalConfigDidChange(_ notification: SwiftUI.Notification) {
            // Get our managed configuration object out
            guard let config = notification.userInfo?[
                SwiftUI.Notification.Name.OrbitTerminalConfigChangeKey
        ...
        @objc private func orbitTerminalColorDidChange(_ notification: SwiftUI.Notification) {
            guard let change = notification.userInfo?[
                SwiftUI.Notification.Name.OrbitTerminalColorChangeKey
            ] as? OrbitTerminal.Action.ColorChange else { return }
        ...
        @objc private func orbitTerminalBellDidRing(_ notification: SwiftUI.Notification) {
            // Bell state goes to true
            bell = true
        }

        @objc private func orbitTerminalDidChangeReadonly(_ notification: SwiftUI.Notification) {
        ...
        override func mouseEntered(with event: NSEvent) {
            ...
            let mouseEvent = OrbitTerminal.Input.MousePosEvent(
                x: pos.x,
        ...
        override func mouseMoved(with event: NSEvent) {
            ...
            if let window,
               ...
               window.isKeyWindow &&
                ...
                OrbitTerminal.moveFocus(to: self)
            }
        }
        ...
        override func pressureChange(with event: NSEvent) {
            ...
            // Notify OrbitTerminal first. We do this because this will let OrbitTerminal handle
        ...
        override func keyDown(with event: NSEvent) {
            ...
            let translationModsOrbitTerminal = OrbitTerminal.eventModifierFlags(
                mods: ghostty_surface_key_translation_mods(
                    surface,
                    OrbitTerminal.ghosttyMods(event.modifierFlags)
                )
            )

            // There are hidden bits set in our event that matter for certain dead keys
            // so we can't use translationModsOrbitTerminal directly. Instead, we just check
            // for exact states and set them.
            var translationMods = event.modifierFlags
            for flag in [NSEvent.ModifierFlags.shift, .control, .option, .command] {
                if translationModsOrbitTerminal.contains(flag) {
                ...
                if keySequence.isEmpty,
                   ...
                   bindingFlags.contains(.consumed) {
                    if let appDelegate = NSApp.delegate as? AppDelegate,
                       appDelegate.performOrbitTerminalBindingMenuKeyEquivalent(with: event) {
                        return true
                    }
                }
        ...
        private func keyAction(
            ...
            // character. Control characters are encoded by OrbitTerminal itself.
        ...
        override func quickLook(with event: NSEvent) {
            ...
            // OrbitTerminal coordinate system is top-left, convert to bottom-left for AppKit
        ...
        func showUserNotification(title: String, body: String, requireFocus: Bool = true) {
            ...
            content.categoryIdentifier = OrbitTerminal.userNotificationCategory
            content.userInfo = [
        ...
        struct DerivedConfig {
            ...
            let scrollbar: OrbitTerminal.Config.Scrollbar
        ...
        required convenience init(from decoder: Decoder) throws {
            // Decoding uses the global OrbitTerminal app
...
extension OrbitTerminal.SurfaceView: NSTextInputClient {
    ...
    func selectedRange() -> NSRange {
        ...
        // Get our range from the OrbitTerminal API. There is a race condition between getting the
    ...
    func attributedSubstring(forProposedRange range: NSRange, actualRange: NSRangePointer?) -> NSAttributedString? {
        // OrbitTerminal.logger.warning("pressure substring range=\(range) selectedRange=\(self.selectedRange())")
        guard let surface = self.surface else { return nil }
    ...
    func firstRect(forCharacterRange range: NSRange, actualRange: NSRangePointer?) -> NSRect {
        ...
        // OrbitTerminal will tell us where it thinks an IME keyboard should render.
        var x: Double = 0
        var y: Double = 0
        ...
        // OrbitTerminal coordinates are in top-left (0, 0) so we have to convert to
...
extension OrbitTerminal.SurfaceView: NSServicesMenuRequestor {
...
extension OrbitTerminal.SurfaceView: NSMenuItemValidation {
...
extension OrbitTerminal.SurfaceView {
    ...
    override func performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        ...
        if let url = pb.string(forType: .URL) {
            ...
            content = OrbitTerminal.Shell.escape(url)
           ...
           urls.count > 0 {
            ...
            content = urls
                .map { OrbitTerminal.Shell.escape($0.path) }
                .joined(separator: " ")
...
extension OrbitTerminal.SurfaceView {
    ...
    /// this to copy styling information as well but we need to augment OrbitTerminal core to
    /// expose that.
    ///
