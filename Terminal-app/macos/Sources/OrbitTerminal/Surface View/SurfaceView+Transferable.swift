...
extension OrbitTerminal.SurfaceView: Transferable {
    static var transferRepresentation: some TransferRepresentation {
        DataRepresentation(contentType: .orbitTerminalSurfaceId) { surface in
    ...
    static func find(uuid: UUID) -> Self? {
        #if canImport(AppKit)
        guard let del = NSApp.delegate as? OrbitTerminal.Delegate else { return nil }
...
extension UTType {
    ...
    static let orbitTerminalSurfaceId = UTType(exportedAs: "com.orbit.orbit-terminalSurfaceId")
}

#if canImport(AppKit)
extension NSPasteboard.PasteboardType {
    /// Pasteboard type for dragging surface IDs.
    static let orbitTerminalSurfaceId = NSPasteboard.PasteboardType(UTType.orbitTerminalSurfaceId.identifier)
}
#endif
