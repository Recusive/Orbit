...
extension OrbitTerminal {
    ...
    struct Terminal: View {
        @EnvironmentObject private var ghostty: OrbitTerminal.App
            ...
            ZStack {
                ...
                if let searchState = surfaceView.searchState {
                    SurfaceSearchOverlay(
                        ...
                        onClose: {
#if canImport(AppKit)
                            OrbitTerminal.moveFocus(to: surfaceView)
#endif
    ...
    struct SurfaceResizeOverlay: View {
        ...
        let overlay: OrbitTerminal.Config.ResizeOverlay
        let position: OrbitTerminal.Config.ResizeOverlayPosition
        let duration: UInt
    ...
    struct SurfaceSearchOverlay: View {
        ...
        var body: some View {
            GeometryReader { geo in
                ...
                .onReceive(NotificationCenter.default.publisher(for: .orbitTerminalSearchFocus)) { notification in
    ...
    /// A surface is terminology in OrbitTerminal for a terminal surface, or a place where a terminal is actually drawn
    ...
    /// libghostty, usually from the OrbitTerminal configuration.
...
private struct OrbitTerminalSurfaceViewKey: EnvironmentKey {
    static let defaultValue: OrbitTerminal.SurfaceView? = nil
}

private struct OrbitTerminalLastFocusedSurfaceKey: EnvironmentKey {
    ...
    static let defaultValue: Weak<OrbitTerminal.SurfaceView>? = nil
}

extension EnvironmentValues {
    var ghosttySurfaceView: OrbitTerminal.SurfaceView? {
        get { self[OrbitTerminalSurfaceViewKey.self] }
        set { self[OrbitTerminalSurfaceViewKey.self] = newValue }
    }

    var ghosttyLastFocusedSurface: Weak<OrbitTerminal.SurfaceView>? {
        get { self[OrbitTerminalLastFocusedSurfaceKey.self] }
        set { self[OrbitTerminalLastFocusedSurfaceKey.self] = newValue }
    }
}

extension View {
    func ghosttySurfaceView(_ surfaceView: OrbitTerminal.SurfaceView?) -> some View {
    ...
    func ghosttyLastFocusedSurface(_ surfaceView: Weak<OrbitTerminal.SurfaceView>?) -> some View {
...
extension FocusedValues {
    var ghosttySurfaceView: OrbitTerminal.SurfaceView? {
        get { self[FocusedOrbitTerminalSurface.self] }
        set { self[FocusedOrbitTerminalSurface.self] = newValue }
    }

    struct FocusedOrbitTerminalSurface: FocusedValueKey {
        typealias Value = OrbitTerminal.SurfaceView
    }

    var ghosttySurfacePwd: String? {
        get { self[FocusedOrbitTerminalSurfacePwd.self] }
        set { self[FocusedOrbitTerminalSurfacePwd.self] = newValue }
    }

    struct FocusedOrbitTerminalSurfacePwd: FocusedValueKey {
        typealias Value = String
    }

    var ghosttySurfaceCellSize: OSSize? {
        get { self[FocusedOrbitTerminalSurfaceCellSize.self] }
        set { self[FocusedOrbitTerminalSurfaceCellSize.self] = newValue }
    }

    struct FocusedOrbitTerminalSurfaceCellSize: FocusedValueKey {
...
extension OrbitTerminal.SurfaceView {
    class SearchState: ObservableObject {
        ...
        init(from startSearch: OrbitTerminal.Action.StartSearch) {
