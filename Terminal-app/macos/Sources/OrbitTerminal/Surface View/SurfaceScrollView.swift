...
/// Wraps a OrbitTerminal surface view in an NSScrollView to provide native macOS scrollbar support.
///
/// ## Coordinate System
...
/// - `surfaceView`: The actual OrbitTerminal renderer, positioned to fill the visible rect
class SurfaceScrollView: NSView {
    private let scrollView: NSScrollView
    private let documentView: NSView
    private let surfaceView: OrbitTerminal.SurfaceView
    ...
    init(contentSize: CGSize, surfaceView: OrbitTerminal.SurfaceView) {
        ...
        // so that our primary OrbitTerminal renderer only needs to render the viewport.
        ...
        // Listen for scrollbar updates from OrbitTerminal
        observers.append(NotificationCenter.default.addObserver(
            forName: .orbitTerminalDidUpdateScrollbar,
            object: surfaceView,
            queue: .main
        ) { [weak self] notification in
    ...
    private func handleScrollbarUpdate(_ notification: Notification) {
        guard let scrollbar = notification.userInfo?[SwiftUI.Notification.Name.ScrollbarKey] as? OrbitTerminal.Action.Scrollbar else {
            return
        }
