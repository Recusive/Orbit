...
protocol OrbitTerminalAppDelegate: AnyObject {
    #if os(macOS)
...
extension OrbitTerminal {
    ...
    // This is a refactor/redo of OrbitTerminal.AppState so that it supports both macOS and iOS
    class App: ObservableObject {
        ...
        /// Optional delegate
        weak var delegate: OrbitTerminalAppDelegate?
        ...
        static func openConfig() {
            let str = OrbitTerminal.AllocatedString(ghostty_config_open_path()).string
        ...
        func reloadConfig(soft: Bool = false) {
            ...
            guard newConfig.loaded else {
                OrbitTerminal.logger.warning("failed to reload configuration")
                return
            }
        ...
        #if os(iOS)
        // MARK: OrbitTerminal Callbacks (iOS)
        ...
        // Called when the selected keyboard changes. We have to notify OrbitTerminal so that
        ...
        // MARK: OrbitTerminal Callbacks (macOS)

        static func closeSurface(_ userdata: UnsafeMutableRawPointer?, processAlive: Bool) {
            let surface = self.surfaceUserdata(from: userdata)
            NotificationCenter.default.post(name: Notification.orbitTerminalCloseSurface, object: surface, userInfo: [
        ...
        static func confirmReadClipboard(
            ...
            guard let request = OrbitTerminal.ClipboardRequest.from(request: request) else { return }
        ...
        static func writeClipboard(
            ...
            let contentArray = (0..<len).compactMap { i in
                OrbitTerminal.ClipboardContent.from(content: content[i])
            }
            ...
            NotificationCenter.default.post(
                ...
                userInfo: [
                    ...
                    Notification.ConfirmClipboardRequestKey: OrbitTerminal.ClipboardRequest.osc_52_write(pasteboard),
                ]
            )
        }
        ...
        /// Determine if a given notification should be presented to the user when OrbitTerminal is running in the foreground.
        ...
        /// Returns the OrbitTerminalState from the given userdata value.
        ...
        static func action(_ app: ghostty_app_t, target: ghostty_target_s, action: ghostty_action_s) -> Bool {
            ...
            default:
                OrbitTerminal.logger.warning("unknown action target=\(target.tag.rawValue)")
                return false
            }
        ...
        private static func openURL(
            ...
            let action = OrbitTerminal.Action.OpenURL(c: v)
        ...
        private static func newSplit(
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    name: Notification.orbitTerminalNewSplit,
                    object: surfaceView,
                    userInfo: [
                        "direction": direction,
        ...
        private static func presentTerminal(
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    name: Notification.orbitTerminalPresentTerminal,
                    object: surfaceView
                )
                return true
        ...
        private static func toggleFullscreen(
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                guard let mode = FullscreenMode.from(ghostty: raw) else {
                    OrbitTerminal.logger.warning("unknown fullscreen mode raw=\(raw.rawValue)")
                    return
                }
                NotificationCenter.default.post(
                    name: Notification.orbitTerminalToggleFullscreen,
                    object: surfaceView,
                    userInfo: [
            ...
            move: ghostty_action_move_tab_s) -> Bool {
                ...
                case GHOSTTY_TARGET_SURFACE:
                    ...
                    NotificationCenter.default.post(
                        ...
                        object: surfaceView,
                        userInfo: [
                            SwiftUI.Notification.Name.OrbitTerminalMoveTabKey: Action.MoveTab(c: move),
                        ]
                    )
        ...
        private static func gotoTab(
            ...
            tab: ghostty_action_goto_tab_e) -> Bool {
                ...
                case GHOSTTY_TARGET_SURFACE:
                    ...
                    NotificationCenter.default.post(
                        name: Notification.orbitTerminalGotoTab,
                        object: surfaceView,
                        userInfo: [
        ...
        private static func gotoSplit(
            ...
            direction: ghostty_action_goto_split_e) -> Bool {
                ...
                case GHOSTTY_TARGET_SURFACE:
                    ...
                    let focusDirection: SplitTree<OrbitTerminal.SurfaceView>.FocusDirection = splitDirection.toSplitTreeFocusDirection()
                    ...
                    NotificationCenter.default.post(
                        name: Notification.orbitTerminalFocusSplit,
                        object: surfaceView,
                        userInfo: [
                ...
                if candidate.isVisible, !candidate.isMiniaturized {
                    ...
                    if let controller = candidate.windowController as? BaseTerminalController,
                       let surface = controller.focusedSurface {
                        OrbitTerminal.moveFocus(to: surface)
                    }
                    return true
                }
        ...
        private static func showDesktopNotification(
            ...
            center.requestAuthorization(options: [.alert, .sound]) { _, error in
                if let error = error {
                    OrbitTerminal.logger.error("Error while requesting notification authorization: \(error)")
                }
            }
        ...
        private static func progressReport(
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                guard config.progressStyle else {
                    OrbitTerminal.logger.debug("progress_report action blocked by config")
                ...
                let progressReport = OrbitTerminal.Action.ProgressReport(c: v)
                DispatchQueue.main.async {
        ...
        private static func scrollbar(
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                let scrollbar = OrbitTerminal.Action.Scrollbar(c: v)
                NotificationCenter.default.post(
                    name: .orbitTerminalDidUpdateScrollbar,
                    object: surfaceView,
                    userInfo: [
        ...
        private static func startSearch(
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                let startSearch = OrbitTerminal.Action.StartSearch(c: v)
                DispatchQueue.main.async {
                    if let searchState = surfaceView.searchState {
                        ...
                        surfaceView.searchState = OrbitTerminal.SurfaceView.SearchState(from: startSearch)
                    }

                    NotificationCenter.default.post(name: .orbitTerminalSearchFocus, object: surfaceView)
                }
            ...
            v: ghostty_action_config_change_s) {
                ...
                case GHOSTTY_TARGET_APP:
                    ...
                    NotificationCenter.default.post(
                        ...
                        object: nil,
                        userInfo: [
                            SwiftUI.Notification.Name.OrbitTerminalConfigChangeKey: config,
                        ]
                    )
            ...
            change: ghostty_action_color_change_s) {
                ...
                case GHOSTTY_TARGET_SURFACE:
                    ...
                    NotificationCenter.default.post(
                        ...
                        object: surfaceView,
                        userInfo: [
                            SwiftUI.Notification.Name.OrbitTerminalColorChangeKey: Action.ColorChange(c: change)
                        ]
                    )
        ...
        func handleUserNotification(response: UNNotificationResponse) {
            ...
            case UNNotificationDefaultActionIdentifier, OrbitTerminal.userNotificationActionShow:
