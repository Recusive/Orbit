...
extension NSEvent {
    /// Create a OrbitTerminal key event for a given keyboard action.
    ///
    ...
    func ghosttyKeyEvent(
        ...
        key_ev.mods = OrbitTerminal.ghosttyMods(modifierFlags)
        key_ev.consumed_mods = OrbitTerminal.ghosttyMods(
    ...
    /// Returns the text to set for a key event for OrbitTerminal.
    ///
    /// This namely contains logic to avoid control characters, since we handle control character
    /// mapping manually within OrbitTerminal.
    var ghosttyCharacters: String? {
        ...
        if characters.count == 1,
           let scalar = characters.unicodeScalars.first {
            ...
            // encoding directly within OrbitTerminal's KeyEncoder.
            if scalar.value < 0x20 {
                return self.characters(byApplyingModifiers: modifierFlags.subtracting(.control))
            }

            // If we have a single value in the PUA, then it's a function key and
            // we don't want to send PUA ranges down to OrbitTerminal.
