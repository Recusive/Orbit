...
# Build the macOS OrbitTerminal app using xcodebuild with a clean environment
# to avoid Nix shell interference (NIX_LDFLAGS, NIX_CFLAGS_COMPILE, etc.).

def main [
    --scheme: string = "OrbitTerminal"       # Xcode scheme (OrbitTerminal, OrbitTerminal-iOS, DockTilePlugin)
    ...
    let project = ($env.FILE_PWD | path join "OrbitTerminal.xcodeproj")
    ...
    let skip_testing = if $action == "test" {
        [-skip-testing OrbitTerminalUITests]
    } else {
        []
    }
