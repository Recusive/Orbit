# macOS Orbit Terminal Application
...
- If code outside of `macos/` directory is modified, use
  ...
  the underlying Orbit Terminal library.
- Use `macos/build.nu` to build the macOS app, do not use `zig build`
  (except to build the underlying library as mentioned above).
  - Build: `macos/build.nu [--scheme Orbit Terminal] [--configuration Debug] [--action build]`
  - Output: `macos/build/<configuration>/Orbit Terminal.app` (e.g. `macos/build/Debug/Orbit Terminal.app`)
- Run unit tests directly with `macos/build.nu --action test`

## AppleScript

- The AppleScript scripting definition is in `macos/Orbit Terminal.sdef`.
...
- In `macos/Orbit Terminal.sdef`, keep top-level definitions in this order:
  1. Classes
  2. Records
  3. Enums
  4. Commands
- Test AppleScript support:
  ...
  `osascript -e 'tell application "<absolute path to build/Debug/Orbit Terminal.app>" to activate'`
  ...
  `osascript -e 'tell application "<absolute path to build/Debug/Orbit Terminal.app>" to quit'`
