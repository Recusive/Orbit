...
struct ColorizedOrbitTerminalIconTests {
    private func makeIcon(
        ...
        frame: OrbitTerminal.MacOSIconFrame = .aluminum
    ) -> ColorizedOrbitTerminalIcon {
    ...
    @Test func codableRoundTripPreservesIcon() throws {
        ...
        let decoded = try JSONDecoder().decode(ColorizedOrbitTerminalIcon.self, from: data)
    ...
    @Test func decodingUnsupportedVersionThrowsDataCorrupted() {
        ...
        do {
            _ = try JSONDecoder().decode(ColorizedOrbitTerminalIcon.self, from: data)
            Issue.record("Expected decode to fail for unsupported version")
        } catch let DecodingError.dataCorrupted(context) {
            #expect(context.debugDescription.contains("Unsupported ColorizedOrbitTerminalIcon version"))
        } catch {
